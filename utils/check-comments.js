...
for (const file of [
  ...
  if ([".sh", ".yml", ".yaml", ".grammar"].includes(extension)) {
    ...
    const found = lines.some((line, index) => {
      ...
      if (extension === ".sh" && index === 0 && value.startsWith("#!")) return false;
      return value.startsWith("#") || extension === ".grammar" && value.startsWith("//");
    });
...
function walk(target, output = []) {
  ...
  for (const entry of fs.readdirSync(target)) walk(path.join(target, entry), output);
  return output;
}
