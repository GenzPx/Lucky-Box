...
const ID_PAID = "id.luckybox.server";
...
try {
	execSync("mkdir -p www/css/build www/js/build", {
		stdio: "inherit",
	});
} catch (error) {
	console.log(
...
execSync("cordova plugin add cordova-plugin-buildinfo", {
	stdio: "inherit",
});
execSync("cordova plugin add cordova-plugin-device", {
	stdio: "inherit",
});
execSync("cordova plugin add cordova-plugin-file", {
	stdio: "inherit",
});
