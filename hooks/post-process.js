...
const androidGradleFilePath = path.resolve(__dirname, '../platforms/android/app/build-extras.gradle');
...
if (!fs.existsSync(copyToPath) && fs.existsSync(buildFilePath)) fs.copyFileSync(buildFilePath, copyToPath);
...
function getPackageName() {
  ...
  const packageName = match ? match[1] : 'id.luckybox.server';
  return packageName;
}
function getTmpDir() {
...
function enableStaticContext() {
  try {
    ...
    const mainActivityPath = path.join(prefix, 'platforms/android/app/src/main/java', packageName.replace(/\./g, '/'), 'MainActivity.java');
    ...
    if (content.includes('WeakReference<Context>') && content.includes('public static Context getContext()') && content.includes('weakContext = new WeakReference<>(this);')) {
      console.log('[Cordova Hook] ✅ Static context already enabled, skipping');
      return;
    }
    if (!content.includes('import java.lang.ref.WeakReference;')) {
      content = content.replace(/import org\.apache\.cordova\.\*;/, match => match + '\nimport android.content.Context;\nimport java.lang.ref.WeakReference;');
    }
    content = content.replace(/public class MainActivity extends CordovaActivity\s*\{/, match => match + `\n\n    private static WeakReference<Context> weakContext;\n\n` + `    public static Context getContext() {\n` + `        return weakContext != null ? weakContext.get() : null;\n` + `    }\n`);
...
function copyDirRecursively(src, dest, skip = [], currPath = '') {
  ...
  if (exists && isDirectory) {
    ...
    fs.readdirSync(src).forEach(childItemName => {
      ...
      copyDirRecursively(path.join(src, childItemName), path.join(dest, childItemName), skip, relativePath);
    });
  } else {
