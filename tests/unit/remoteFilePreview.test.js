...
function createCache({
  ...
  return {
    ...
    readFile: error ? vi.fn().mockRejectedValue(error) : vi.fn().mockResolvedValue(text)
  };
}
describe("readRemoteFilePreview", () => {
  it("prefers editor recovery data over the transport cache", async () => {
    ...
    await expect(readRemoteFilePreview({
      editorCache,
      transportCache,
      encoding: "UTF-8"
    })).resolves.toEqual({
      editorCacheExists: true,
      text: "unsaved"
    });
