...
import quickToolsModifierInput, { isSelectedRangeDeletion, setQuickToolsModifierInputHandler, type QuickToolsModifierInputContext } from "cm/quickToolsModifierInput";
function createView(doc = "abcdef", selection = {
  anchor: 1,
  head: 4
}, extraExtensions = []) {
...
function applyInput(view: EditorView, input: QuickToolsModifierInputContext): boolean {
  const defaultInsert = () => view.state.update({
    ...
    selection: {
      anchor: input.from + input.text.length
    }
  });
  const handled = view.state.facet(EditorView.inputHandler).some(handler => handler(view, input.from, input.to, input.text, defaultInsert));
...
describe("quick-tools modifier input", () => {
  it("recognizes only empty changes that cover the active selection", () => {
    ...
    expect(isSelectedRangeDeletion(view, {
      from: 1,
      to: 4,
      text: ""
    })).toBe(true);
  ...
  it("preserves selection across Android delete-then-copy input", () => {
    ...
    setQuickToolsModifierInputHandler((target, input) => {
      ...
      copied = target.state.sliceDoc(target.state.selection.main.from, target.state.selection.main.to);
      ctrlArmed = false;
      return true;
    });
    expect(applyInput(view, {
      from: 0,
      to: doc.length,
      text: ""
    })).toBe(true);
  ...
  it("lets CodeMirror restore a handled syntax-highlighted DOM replacement", async () => {
    ...
    const stringSpan = Array.from(view.contentDOM.querySelectorAll("span")).find(element => element.textContent?.includes("codemirror"));
