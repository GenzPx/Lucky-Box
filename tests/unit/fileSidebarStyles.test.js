...
const stylesheetPath = path.resolve(process.cwd(), "src/sidebarApps/files/style.scss");
...
describe("file sidebar label overflow", () => {
  it("truncates root folder names without changing nested row scrolling", () => {
    const {
      document,
      window
    } = renderFileTreeStyles();
    const rootStyles = window.getComputedStyle(document.getElementById("root-folder-name"));
    const nestedStyles = window.getComputedStyle(document.getElementById("nested-file-name"));
