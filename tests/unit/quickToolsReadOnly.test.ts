...
import quickToolsModifierInput, { canQuickToolsEdit, finishQuickToolsModifierInput, focusQuickToolsModifierInput, setQuickToolsModifierInputHandler } from "cm/quickToolsModifierInput";
import { runQuickToolKey } from "cm/quickToolsNavigation";
import { captureReadOnlyQuickToolsKey, createReadOnlyQuickToolsCaptureSession } from "handlers/readOnlyQuickToolsCapture";
import { afterEach, describe, expect, it, vi } from "vitest";
vi.mock("utils/keyboardEvent", () => ({
  default: (type: string, init: KeyboardEventInit) => new KeyboardEvent(type, init)
}));
...
describe("read-only QuickTools interaction", () => {
  it("selects all without focusing or changing the document", () => {
    const view = createEditor(true, [keymap.of([{
      key: "Ctrl-a",
      run: selectAll
    }])]);
    const originalDocument = view.state.doc.toString();
    expect(runQuickToolKey(view, 65, {
      ctrlKey: true
    })).toBe(true);
  ...
  it("allows selection navigation but blocks deletion", () => {
    ...
    expect(runQuickToolKey(view, 39, {
      shiftKey: true
    })).toBe(true);
