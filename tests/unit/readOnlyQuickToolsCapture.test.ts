...
describe("read-only QuickTools capture", () => {
  it("declines editable targets", () => {
    expect(createReadOnlyQuickToolsCaptureSession({
      state: {
        readOnly: false
      }
    }, modifiers)).toBeNull();
  ...
  it.each([["keydown", {
    ...
    const session = createReadOnlyQuickToolsCaptureSession(readOnlyTarget(), modifiers);
  ...
  it("captures the first usable character from an Android composition", () => {
    ...
    if (session) {
      ...
      expect(processKey.outcome).toEqual({
        kind: "pending"
      });
      ...
      expect(beforeInput.outcome).toEqual({
        kind: "key",
        key: "a"
      });
