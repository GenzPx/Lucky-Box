...
import { clearModifierState, clearQuickToolsButtonFeedback, modifierKeys, removeActionStackEntries, shouldCaptureModifierInput } from "handlers/quickToolsState";
describe("shouldCaptureModifierInput", () => {
  ...
  it("captures command modifiers outside CodeMirror", () => {
    expect(shouldCaptureModifierInput(modifiers({
      shift: true
    }), false)).toBe(true);
    expect(shouldCaptureModifierInput(modifiers({
      ctrl: true
  ...
  it.each(["ctrl", "alt", "meta"])("captures CodeMirror input when %s is active", key => {
    expect(shouldCaptureModifierInput(modifiers({
      [key]: true
    }), true)).toBe(true);
  });
...
describe("clearModifierState", () => {
  ...
  it("emits false for every registered listener", () => {
    ...
    expect(emitted).toEqual([["shift", false], ["alt", false], ["ctrl", false], ["meta", false]]);
  });
