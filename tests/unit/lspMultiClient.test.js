...
import { LSPClient, LSPPlugin, Workspace, serverCompletionSource, serverDiagnostics } from "@codemirror/lsp-client";
import { getLspDiagnostics, lspDiagnosticsAutoSyncExtension, lspDiagnosticsClientExtension, lspDiagnosticsUiExtension, schedulePullDiagnostics } from "cm/lsp/diagnostics";
...
class TestWorkspace extends Workspace {
  ...
  closeFile(uri, view) {
    const index = this.files.findIndex(file => file.uri === uri && file.getView() === view);
    if (index >= 0) {
      this.closed++;
...
function client() {
  return new LSPClient({
    workspace: lspClient => new TestWorkspace(lspClient)
  });
}
...
describe("multiple LSP clients on one editor", () => {
  it("tracks one change stream with an independent sync checkpoint per client", () => {
    ...
    const primaryBinding = primary.plugin(uri, "typescript", {
      priority: 100
    });
    ...
    expect(LSPPlugin.getAll(view).map(plugin => plugin.client)).toEqual([primary, supplemental]);
    ...
    expect(LSPPlugin.get(view, primary).unsyncedChanges.apply(primary.workspace.files[0].doc).toString()).toBe(firstText);
    expect(LSPPlugin.get(view, supplemental).unsyncedChanges.apply(supplemental.workspace.files[0].doc).toString()).toBe(firstText);
    ...
    expect(LSPPlugin.get(view, supplemental).unsyncedChanges.apply(supplemental.workspace.files[0].doc).toString()).toBe(view.state.doc.toString());
  ...
  it("merges completion providers by priority and isolates failures", async () => {
    ...
    supplemental.serverCapabilities = {
      completionProvider: {}
    };
    failing.serverCapabilities = {
      completionProvider: {}
    };
    primary.sync = vi.fn();
    ...
    const view = editor("di", [primary.plugin("file:///workspace/example.tsx", "typescriptreact", {
      priority: 100
    ...
    const context = new CompletionContext(view.state, view.state.doc.length, true, view);
    const result = await serverCompletionSource(context);
    expect(result.options.map(option => option.label)).toEqual(["typescriptItem", "sharedItem", "tailwindItem"]);
    ...
    expect(warn).toHaveBeenCalledWith("[lsp] Completion provider failed", expect.objectContaining({
  ...
  it("does not query a client when completion is disabled for its binding", async () => {
    ...
    diagnosticsOnly.serverCapabilities = {
      completionProvider: {}
    };
    primary.sync = vi.fn();
    ...
    diagnosticsOnly.request = vi.fn().mockResolvedValue([{
      label: "wrongItem"
    }]);
    const view = editor("x", [primary.plugin("file:///workspace/example.ts", "typescript"), diagnosticsOnly.plugin("file:///workspace/example.ts", "typescript", {
      features: {
        completion: false
      }
    })]);
    const result = await serverCompletionSource(new CompletionContext(view.state, 1, true, view));
  ...
  it("keeps diagnostics from each server when another server publishes", () => {
    ...
    const view = editor("const x = 1", [primary.plugin("file:///workspace/example.ts", "typescript"), supplemental.plugin("file:///workspace/example.ts", "typescript"), diagnosticsExtension.editorExtension]);
    const publish = diagnosticsExtension.notificationHandlers["textDocument/publishDiagnostics"];
  ...
  it("does not synchronize a server whose diagnostics feature is disabled", async () => {
    ...
    disabled.sync = vi.fn();
    const uri = "file:///workspace/example.ts";
    const view = editor("const x = 1", [enabled.plugin(uri, "typescript"), disabled.plugin(uri, "typescript", {
      features: {
        diagnostics: false
      }
  ...
  it("rejects pull and push diagnostics from a disabled provider", () => {
    const disabled = client();
    disabled.serverCapabilities = {
      diagnosticProvider: {}
    };
    disabled.request = vi.fn();
    const uri = "file:///workspace/example.ts";
    const view = editor("const x = 1", [disabled.plugin(uri, "typescript", {
      features: {
        diagnostics: false
      }
    ...
    const publish = lspDiagnosticsClientExtension().notificationHandlers["textDocument/publishDiagnostics"];
    publish(disabled, {
      uri,
      version: 0,
      diagnostics: [{
