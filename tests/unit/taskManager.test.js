...
describe("TaskManager (linear)", () => {
  it("runs tasks one at a time in queue order", async () => {
    ...
    const results = await Promise.all([tm.queueTask(task("a", 30)), tm.queueTask(task("b", 10)), tm.queueTask(task("c", 0))]);
    expect(order).toEqual(["a", "b", "c"]);
    expect(results).toEqual(["a", "b", "c"]);
  });
  it("passes an incrementing id to each task", async () => {
    const tm = new TaskManager("linear");
    const ids = await Promise.all([tm.queueTask(async id => id), tm.queueTask(async id => id), tm.queueTask(async id => id)]);
...
describe("TaskManager (parallel)", () => {
  it("runs tasks concurrently", async () => {
    ...
    const order = [];
    await Promise.all([tm.queueTask(async () => {
      await sleep(30);
      order.push("slow");
    }), tm.queueTask(async () => {
      order.push("fast");
  ...
  it("propagates errors to the awaiting caller", async () => {
    const tm = new TaskManager("parallel");
    await expect(tm.queueTask(async () => {
      throw new Error("boom");
