...
describe("language mode recommendations", () => {
  ...
  it.each([["network errors", () => Promise.reject(new Error("offline"))], ["server errors", () => Promise.resolve({
    ok: false,
    status: 503
  })]])("retries after transient %s", async (_, failedResponse) => {
    globalThis.fetch = vi.fn().mockImplementationOnce(failedResponse).mockResolvedValueOnce({
      ok: true,
      json: async () => [{
        id: "recovered-language-mode"
      }]
    });
