...
describe("copyEntry", () => {
  ...
  it("copies excluded paths when no patterns are enabled", async () => {
    ...
    expect(fileSystem.created.map(({
      url
    }) => url)).toEqual(["/target/node_modules", "/target/node_modules/package.json"]);
  });
  it("does not prepare or replace the target for an excluded source", async () => {
    const onBeforeCopy = vi.fn();
    const result = await copyEntry("/source/node_modules/package.json", "/target", {
      excludePatterns: ["**/node_modules/**"],
      onBeforeCopy
    });
    expect(result).toEqual({
