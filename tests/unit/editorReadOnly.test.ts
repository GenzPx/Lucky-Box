...
import { createEditorReadOnlyExtension, focusEditorIfEditable, isReadOnlyUserChange, placeReadOnlyCursor, reconfigureEditorReadOnly, resolveReadOnlyContextSelection, shouldCommitReadOnlyTap } from "cm/editorReadOnly";
...
function createEditor(readOnly = false, doc = "read only content") {
  ...
  const state = EditorState.create({
    doc,
    extensions: [drawSelection(), compartment.of(createEditorReadOnlyExtension(readOnly))]
  });
...
describe("editor read-only configuration", () => {
  ...
  it("blocks user document changes but permits internal synchronization", () => {
    ...
    expect([view.state.selection.main.from, view.state.selection.main.to]).toEqual([0, 4]);
    view.dispatch({
      changes: {
        from: 0,
        to: 4,
  ...
  it("accepts only uncancelled short primary taps", () => {
    ...
    expect(shouldCommitReadOnlyTap({
      ...start,
      cancelled: true
    }, end)).toBe(false);
    expect(shouldCommitReadOnlyTap(start, {
      ...end,
      x: 50
    })).toBe(false);
    expect(shouldCommitReadOnlyTap(start, {
