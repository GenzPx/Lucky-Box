...
const TREE_URI = "content://com.android.externalstorage.documents/tree/primary%3ADCIM";
const SINGLE_URI = "content://com.android.externalstorage.documents/document/primary%3ADCIM%2Ffile.txt";
describe("Uri.parse", () => {
  ...
  it("converts a single document uri into tree form", () => {
    expect(Uri.parse(SINGLE_URI)).toEqual({
      rootUri: "content://com.android.externalstorage.documents/tree/primary%3A",
      docId: "primary:DCIM/file.txt",
      isFileUri: false
    });
  });
...
describe("Uri.format", () => {
  it("joins rootUri and docId with '::'", () => {
    expect(Uri.format(TREE_URI, "primary:DCIM/file.txt")).toBe(`${TREE_URI}::primary:DCIM/file.txt`);
  });
...
describe("Uri.getPrimaryAddress", () => {
  it("returns the path after '::primary:'", () => {
    expect(Uri.getPrimaryAddress(`${TREE_URI}::primary:DCIM/foo`)).toBe("DCIM/foo");
  });
});
describe("Uri.getDisplayPath", () => {
  it("uses the storage name and a path relative to its SAF root", () => {
    ...
    expect(Uri.getDisplayPath(url, [{
      name: "Pictures",
      url: TREE_URI
    }])).toBe("Pictures/Camera/photo.jpg");
  });
  it("does not match aliases from a prefix-sharing SAF root", () => {
    const codesRoot = "content://com.android.externalstorage.documents/tree/primary%3ACodes";
    const codesBackupRoot = "content://com.android.externalstorage.documents/tree/primary%3ACodesBackup";
    expect(Uri.getDisplayPath(codesBackupRoot, [{
      name: "Codes",
      url: codesRoot
    }])).toBe("primary/CodesBackup");
  });
  it("keeps direct children of a volume-root storage", () => {
    const rootUri = "content://com.android.externalstorage.documents/tree/primary%3A";
  ...
  it("falls back to a readable volume and document path", () => {
    expect(Uri.getDisplayPath(SINGLE_URI, [])).toBe("primary/DCIM/file.txt");
  });
  it("includes the selected folder in a bare primary tree path", () => {
    const url = "content://com.android.externalstorage.documents/tree/primary%3ACodes";
    expect(Uri.getDisplayPath(url, [])).toBe("primary/Codes");
  });
  it("preserves an absolute Termux tree path", () => {
    const url = "content://com.termux.documents/tree/%2Fdata%2Fdata%2Fcom.termux%2Ffiles%2Fhome";
    expect(Uri.getDisplayPath(url, [])).toBe("/data/data/com.termux/files/home");
  });
  it("preserves an absolute path appended to a Termux tree", () => {
    const root = "content://com.termux.documents/tree/%2Fdata%2Fdata%2Fcom.termux%2Ffiles%2Fhome";
    const url = `${root}::/data/data/com.termux/files/home/acode-site-ui`;
    expect(Uri.getDisplayPath(url, [])).toBe("/data/data/com.termux/files/home/acode-site-ui");
  });
});
