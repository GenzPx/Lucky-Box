...
describe("Url.basename / extname", () => {
  it("returns the last segment of a url", () => {
    expect(Url.basename("ftp://localhost/foo/bar/index.html")).toBe("index.html");
  });
  it("ignores trailing slashes", () => {
    expect(Url.basename("https://example.com/foo/")).toBe("foo");
  });
  it("resolves basenames of content uris", () => {
    expect(Url.basename("content://com.android.externalstorage.documents/tree/primary%3ADCIM::primary:DCIM/file.txt")).toBe("file.txt");
  });
...
describe("Url.join", () => {
  it("joins path segments keeping the protocol", () => {
    expect(Url.join("ftp://localhost/foo/", "bar", "baz.txt")).toBe("ftp://localhost/foo/bar/baz.txt");
    expect(Url.join("https://example.com", "/a/b")).toBe("https://example.com/a/b");
  });
...
describe("Url.parse / pathname / dirname", () => {
  ...
  it("returns the parent url", () => {
    expect(Url.dirname("ftp://localhost/foo/bar")).toBe("ftp://localhost/foo/");
  });
});
describe("Url.safe", () => {
  it("encodes path segments but leaves the query untouched", () => {
    expect(Url.safe("https://ex.com/a b/c?x=1 2")).toBe("https://ex.com/a%20b/c?x=1 2");
  });
});
...
describe("Url.hidePassword / decodeUrl", () => {
  it("strips the password from a url", () => {
    expect(Url.hidePassword("ftp://user:secret@host.com/a")).toBe("ftp://user@host.com/a");
  });
  ...
  it("decodes credentials, port, path and query", () => {
    expect(Url.decodeUrl("http://user:p%40ss@host.com:8080/a%20b?x=1")).toEqual({
      username: "user",
      password: "p@ss",
      hostname: "host.com",
      pathname: "/a b",
