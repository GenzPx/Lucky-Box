...
const VALID_CONSENT_STATUSES = new Set(["unknown", "required", "notRequired", "obtained"]);
function normalizePrivacyState(state) {
  const consentStatus = VALID_CONSENT_STATUSES.has(state?.consentStatus) ? state.consentStatus : "unknown";
  return {
    consentStatus,
