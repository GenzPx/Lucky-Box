...
function slugifyModeId(value: string): string {
  return normalizeModeKey(value).replace(/\+\+/g, "pp").replace(/#/g, "sharp").replace(/&/g, "and").replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "");
}
function collectAliases(name: string, aliases: readonly string[] | undefined): string[] {
  return [...new Set([name, ...(aliases || [])].map(normalizeModeKey).filter(Boolean))];
}
function getModeId(name: string, aliases: string[]): string {
  ...
  const safeAlias = aliases.find(alias => alias !== normalizedName && isSafeModeId(alias));
    ...
    case "vue":
      return async () => {
        const [{
          ...
          html
        }] = await Promise.all([import("@codemirror/lang-vue"), import("@codemirror/lang-html")]);
        return vue({
          base: html({
            autoCloseTags: await shouldAutoCloseTags()
          })
    ...
    case "angular":
      return async () => {
        const [{
          ...
          html
        }] = await Promise.all([import("@codemirror/lang-angular"), import("@codemirror/lang-html")]);
        return angular({
          base: html({
            autoCloseTags: await shouldAutoCloseTags(),
    ...
    case "php":
      return async () => {
        const [{
          ...
          html
        }] = await Promise.all([import("@codemirror/lang-php"), import("@codemirror/lang-html")]);
...
for (const lang of languages as readonly LanguageDescription[]) {
  try {
    ...
    const filenames = [...(Array.isArray(lang.filenames) ? lang.filenames : lang.filename ? [lang.filename] : [])];
    const normalizedName = name.toLowerCase();
    if (normalizedName === "shell" || modeId === "shell") {
      langExtensions.push("zsh", "zshrc", "bashrc", "bash_profile", "zprofile", "zlogin", "zlogout", "zshenv", "profile", "bash_login", "bash_logout", "zunit", "ztst", "zsh-theme", "bash_history", "zsh_history");
      filenames.push(".bashrc", ".zshrc", ".bash_profile", ".zprofile", ".zlogin", ".zlogout", ".zshenv", ".profile", ".bash_login", ".bash_logout", ".bash_history", ".zsh_history");
    } else if (normalizedName === "properties files" || normalizedName === "properties" || modeId === "ini") {
      langExtensions.push("gitconfig", "gitmodules", "editorconfig", "npmrc", "yarnrc");
      filenames.push(".gitconfig", ".gitmodules", ".editorconfig", ".npmrc", ".yarnrc");
...
addMode("Luau", "luau", "Luau", async () => {
  const {
    luau
  } = await import("./modes/luau");
  return luau();
});
addMode("Astro", "astro", "Astro", async () => {
  const {
    astro
  } = await import("./modes/astro");
  return astro({
    autoCloseTags: await shouldAutoCloseTags()
  });
});
