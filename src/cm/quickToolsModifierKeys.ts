...
export function getQuickToolCombo(key: string, modifiers: QuickToolModifiers = {}): string | null {
  const normalizedKey = normalizeKey(key);
  if (!normalizedKey) return null;
  const modifierParts = Object.entries(modifiers).filter(([, enabled]) => enabled).map(([modifier]) => modifier.replace(/Key$/, ""));
  return normalizeShortcutCombo([...modifierParts, normalizedKey].join("-"));
}
export function findQuickToolCommand(commands: QuickToolCommand[], key: string, modifiers: QuickToolModifiers = {}): QuickToolCommand | null {
  const combo = getQuickToolCombo(key, modifiers);
  if (!combo) return null;
  return commands.find(command => getShortcutAlternatives(command.key).includes(combo)) || null;
}
export function getShortcutAlternatives(keyString: string | null | undefined): string[] {
  if (!keyString) return [];
  return String(keyString).split("|").map(normalizeShortcutCombo).filter((combo): combo is string => Boolean(combo));
}
