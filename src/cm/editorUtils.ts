...
export function setScrollPosition(view: EditorView, scrollTop?: number, scrollLeft?: number): void {
...
export function restoreSelection(view: EditorView, sel: SerializedSelection | null | undefined): void {
  ...
  const mainIndex = sel.mainIndex >= 0 && sel.mainIndex < ranges.length ? sel.mainIndex : 0;
  view.dispatch({
    selection: EditorSelection.create(ranges, mainIndex),
    scrollIntoView: true
  });
}
export function restoreFolds(view: EditorView, folds: FoldSpan[] | null | undefined): void {
  if (!Array.isArray(folds) || folds.length === 0) return;
  function lineColToOffset(doc: EditorState["doc"], line: number, col: number): number {
    const ln = doc.line(line);
    return Math.min(ln.from + col, ln.to);
  }
  function loadFolds(state: EditorState, saved: FoldSpan[]): StateEffect<{
    from: number;
    to: number;
  }>[] {
    const doc = state.doc;
