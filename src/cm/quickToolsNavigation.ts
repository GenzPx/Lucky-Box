...
export function createQuickToolKeyEvent(keyCode: number, modifiers: QuickToolKeyModifiers = {}): KeyboardEvent {
...
export function runQuickToolKey(view: CodeMirrorEditorView, keyCode: number, modifiers: QuickToolKeyModifiers = {}): boolean {
...
function getFallbackCommand(keyCode: number, modifiers: QuickToolKeyModifiers = {}): Command | undefined {
  if (keyCode === 46) return getDeleteForwardCommand(modifiers);
  if (keyCode === 8) return getDeleteBackwardCommand(modifiers);
  if (modifiers.metaKey) {
    return modifiers.shiftKey ? metaShiftMovementCommands[keyCode] : metaMovementCommands[keyCode];
  }
  if (modifiers.ctrlKey || modifiers.altKey) {
    return modifiers.shiftKey ? ctrlShiftMovementCommands[keyCode] : ctrlMovementCommands[keyCode];
  }
  return modifiers.shiftKey ? shiftMovementCommands[keyCode] : plainMovementCommands[keyCode];
}
function getDeleteForwardCommand(modifiers: QuickToolKeyModifiers = {}): Command {
...
function getDeleteBackwardCommand(modifiers: QuickToolKeyModifiers = {}): Command {
