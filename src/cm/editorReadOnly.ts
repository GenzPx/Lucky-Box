import { Compartment, EditorSelection, EditorState, Transaction, type Extension, type SelectionRange } from "@codemirror/state";
...
const readOnlyUserChangeFilter = EditorState.transactionFilter.of(transaction => {
...
const readOnlyCursor = [EditorView.editorAttributes.of({
  class: "cm-read-only"
}), EditorView.theme({
...
export function createEditorReadOnlyExtension(readOnly: boolean): Extension {
  return [EditorState.readOnly.of(readOnly), EditorView.editable.of(!readOnly), ...(readOnly ? [readOnlyFocusGuard, readOnlyInputGuard, readOnlyUserChangeFilter, readOnlyCursor] : [])];
}
...
export function blurEditorIfReadOnly(view: EditorView, readOnly: boolean): void {
  if (!readOnly) return;
...
export function reconfigureEditorReadOnly(view: EditorView, compartment: Compartment, readOnly: boolean): void {
  view.dispatch({
...
export function shouldCommitReadOnlyTap(start: PointerTapSnapshot, end: PointerTapEnd, options: PointerTapOptions = {}): boolean {
  const {
    maxDelay = 500,
    maxDistance = 20
  } = options;
...
export function resolveReadOnlyContextSelection(state: EditorState, pos: number): SelectionRange {
  const docLength = state.doc.length;
  const position = Math.max(0, Math.min(Number.isFinite(pos) ? pos : state.selection.main.head, docLength));
  ...
  const codePoint = state.doc.sliceString(from, Math.min(from + 2, docLength)).codePointAt(0);
