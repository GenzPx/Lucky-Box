...
const controlKeywords = new Set(["break", "continue", "do", "else", "elseif", "end", "for", "function", "if", "in", "repeat", "return", "then", "type", "until", "while"]);
const modifierKeywords = new Set(["export", "local"]);
const logicalKeywords = new Set(["and", "not", "or"]);
const typePrimitives = new Set(["any", "boolean", "buffer", "never", "nil", "number", "string", "symbol", "thread", "unknown", "userdata", "vector"]);
const standardFunctions = new Set(["assert", "collectgarbage", "delay", "error", "gcinfo", "getfenv", "getmetatable", "ipairs", "loadstring", "newproxy", "next", "pairs", "pcall", "print", "printidentity", "rawequal", "rawset", "require", "select", "setfenv", "setmetatable", "settings", "spawn", "stats", "tick", "time", "tonumber", "tostring", "type", "typeof", "unpack", "UserSettings", "version", "wait", "warn"]);
const standardNamespaces = new Set(["bit32", "buffer", "coroutine", "debug", "math", "os", "string", "table", "task", "utf8", "vector", "Enum"]);
const standardVariables = new Set(["_G", "_VERSION", "DebuggerManager", "PluginManager", "game", "plugin", "script", "shared", "workspace"]);
const metamethods = new Set(["__add", "__call", "__concat", "__div", "__eq", "__idiv", "__index", "__iter", "__le", "__len", "__lt", "__metatable", "__mod", "__mode", "__mul", "__newindex", "__pow", "__sub", "__tostring", "__unm"]);
const typeTerminators = new Set(["break", "continue", "do", "else", "elseif", "end", "for", "if", "in", "local", "repeat", "return", "then", "until", "while"]);
...
function looksLikeMethodSeparator(stream: StringStream) {
  return /^\s*[A-Za-z_][A-Za-z0-9_]*\s*\(/.test(stream.string.slice(stream.pos));
}
...
const docCommentLine: Tokenizer = (stream, state) => {
  ...
  if (state.docCommentExpectType && (isWordStart(peek) || peek === "{" || peek === "(" || peek === "[" || peek === "?" || peek === "." || peek === "|")) {
    stream.next();
    stream.eatWhile(/[^\s,;]+/);
    state.docCommentExpectType = false;
    return "typeName";
...
function classifyIdentifier(word: string, state: LuauState) {
  if (state.expectFunctionName && isWordStart(word)) {
    ...
    return metamethods.has(word) ? "variableName.function.definition.special" : "variableName.function.definition";
  }
...
const normal: Tokenizer = (stream, state) => {
  ...
  if (char === "." || char === ":") {
    ...
    if (char === ":" && !state.expectFunctionName && !looksLikeMethodSeparator(stream)) {
      enterTypeContext(state);
      state.lastIdentifierWasStandard = false;
      return "operator";
    }
  ...
  if (char === "<" && (state.afterTypeName || state.afterFunctionName || state.afterTypeIdentifier)) {
    enterTypeContext(state);
    state.genericDepth++;
    state.afterFunctionName = false;
    state.afterTypeIdentifier = false;
...
const luauLanguage = StreamLanguage.define<LuauState>({
  ...
  indent(state, textAfter, context: IndentContext) {
    ...
    return state.basecol + context.unit * (state.indentDepth - (closing ? 1 : 0));
  },
  languageData: {
    commentTokens: {
      line: "--",
