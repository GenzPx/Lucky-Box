...
import { ExpressionClose, ExpressionContent, ExpressionOpen, FrontmatterClose, FrontmatterContent, FrontmatterOpen, HtmlContent } from "./parser.terms";
type Input = {
  next: number;
  pos: number;
...
function isNameBoundary(code: number): boolean {
  return code === eof || code === space || code === tab || code === newline || code === carriageReturn || code === slash || code === greaterThan;
}
...
function lineHasOnlyFrontmatterFence(input: Input, offset = 0): boolean {
  if (input.peek(offset) !== dash || input.peek(offset + 1) !== dash || input.peek(offset + 2) !== dash) {
    return false;
  }
...
function startsWithTagName(input: Input, offset: number, name: string): boolean {
...
function tagEnd(input: Input, offset: number): TagEnd {
  let quote = 0;
  ...
  for (let i = offset;; i++) {
    ...
    if (next === greaterThan) {
      return {
        ...
        selfClosing: lastNonWhitespace === slash
      };
    }
...
function closeTagEndOffset(input: Input, offset: number, name: string): number {
...
function rawTextElementEndOffset(input: Input, offset: number): number | null {
...
function htmlCommentEndOffset(input: Input, offset: number): number | null {
  if (input.peek(offset) !== lessThan || input.peek(offset + 1) !== exclamation || input.peek(offset + 2) !== dash || input.peek(offset + 3) !== dash) {
    return null;
  }
  for (let i = offset + 4;; i++) {
    const next = input.peek(i);
    if (next === eof) return i;
    if (next === dash && input.peek(i + 1) === dash && input.peek(i + 2) === greaterThan) {
      return i + 3;
    }
  }
}
function expressionEndOffset(input: Input, offset: number): number {
  let depth = 0;
  let quote = 0;
  let escaped = false;
  ...
  for (let i = offset;; i++) {
    ...
    if (char === closeBrace) {
      const templateExpressionDepth = templateExpressionDepths[templateExpressionDepths.length - 1];
...
function htmlContentEndOffset(input: Input): number {
...
function frontmatterContentEndOffset(input: Input): number {
  for (let offset = 0;; offset++) {
    const next = input.peek(offset);
    if (next === eof) return offset;
    if ((offset === 0 || input.peek(offset - 1) === newline) && lineHasOnlyFrontmatterFence(input, offset)) {
      return offset;
    }
  }
}
export const astroTokens = new ExternalTokenizer((input, stack) => {
  const startOffset = input.next === bom ? 1 : 0;
  if (stack.canShift(FrontmatterOpen) && (input.pos === 0 || input.pos === 1 && input.peek(-1) === bom) && lineHasOnlyFrontmatterFence(input, startOffset)) {
    input.acceptTokenTo(FrontmatterOpen, lineEndPosition(input));
    return;
  }
  if (stack.canShift(FrontmatterClose) && atLineStart(input) && lineHasOnlyFrontmatterFence(input)) {
