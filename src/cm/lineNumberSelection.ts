...
type LineNumberClickEvent = Pick<MouseEvent, "button" | "shiftKey" | "altKey" | "ctrlKey" | "metaKey" | "preventDefault" | "defaultPrevented">;
interface LineNumberClickOptions {
  shiftClickSelection?: boolean;
}
function toDocumentOffset(value: number | null | undefined, fallback = 0): number {
  const resolved = value != null ? Number(value) : fallback;
  return Number.isFinite(resolved) ? resolved : fallback;
}
export function getLineSelectionRange(state: EditorView["state"], line: LineInfo): {
  from: number;
  to: number;
} | null {
  if (!line) return null;
...
function getCurrentSelectionLineRange(state: EditorView["state"]): {
  ...
  const endPos = selection.empty ? selection.head : Math.max(selection.to - 1, selection.from);
...
function createExtendedLineSelection(state: EditorView["state"], clickedRange: {
  from: number;
  to: number;
}): EditorSelection {
...
export function handleLineNumberClick(view: EditorView | null | undefined, line: LineInfo, event: LineNumberClickEvent | null | undefined, options: LineNumberClickOptions = {}): boolean {
  ...
  const extendSelection = event.shiftKey && options.shiftClickSelection !== false;
  event.preventDefault();
  view.dispatch({
    selection: extendSelection ? createExtendedLineSelection(view.state, range) : createLineSelection(range),
