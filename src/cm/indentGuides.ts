...
function buildGuideStyle(levels: number, guideStepPx: number): string {
  ...
  return [`background-image:${images.join(",")}`, "background-repeat:no-repeat", `background-position:${positions.join(",")}`, `background-size:${sizes.join(",")}`].join(";");
}
function getGuideStyle(levels: number, guideStepPx: number, styleCache: GuideStyleCache): string {
...
function getCachedLineInfo(lineNumber: number, lineText: string, tabSize: number, cache: IndentLineCache): IndentLineInfo {
...
function buildDecorations(view: EditorView, config: Required<IndentGuidesConfig>, lineCache: IndentLineCache, styleCache: GuideStyleCache): DecorationSet {
  ...
  for (const {
    ...
    const scanEndLine = Math.min(state.doc.lines, lastLineNumber + BLANK_LINE_SCAN_LIMIT);
    ...
    for (let lineNum = scanStartLine; lineNum <= scanEndLine; lineNum++) {
      ...
      prevIndentByLine.set(lineNum, lineNum - prevIndentLine <= BLANK_LINE_SCAN_LIMIT ? prevIndent : -1);
      if (!info.blank) {
    ...
    for (let lineNum = scanEndLine; lineNum >= scanStartLine; lineNum--) {
      const line = state.doc.line(lineNum);
      const info = getCachedLineInfo(lineNum, line.text, tabSize, lineCache);
      nextIndentByLine.set(lineNum, nextIndentLine - lineNum <= BLANK_LINE_SCAN_LIMIT ? nextIndent : -1);
      if (!info.blank) {
    ...
    for (let lineNum = firstLineNumber; lineNum <= lastLineNumber; lineNum++) {
      ...
      const levels = Math.min(Math.floor(indentColumns / indentUnit), MAX_GUIDE_LEVELS);
      ...
      if (info.blank) {
        ...
        builder.add(line.from, line.from + info.leadingWhitespaceLength, Decoration.mark({
          attributes: {
...
function createIndentGuidesPlugin(config: Required<IndentGuidesConfig>): ViewPlugin<{
  ...
  return ViewPlugin.fromClass(class {
    ...
    constructor(view: EditorView) {
      ...
      this.decorations = buildDecorations(view, config, this.lineCache, this.styleCache);
    }
    update(update: ViewUpdate): void {
      const {
        view,
      ...
      if (currentTabSize !== this.lastTabSize || currentIndentUnit !== this.lastIndentUnit) {
...
export function indentGuidesExtension(enabled: boolean, config: IndentGuidesConfig = {}): Extension {
  if (!enabled) return [];
