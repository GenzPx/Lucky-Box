...
const DEFAULT_DARK_COLORS = ["#e5c07b", "#c678dd", "#56b6c2", "#61afef", "#98c379", "#d19a66"];
const DEFAULT_LIGHT_COLORS = ["#795e26", "#af00db", "#005cc5", "#008000", "#b15c00", "#267f99"];
...
function clampLookBehind(value: number | undefined): number {
  ...
  return Math.max(MIN_LOOK_BEHIND, Math.min(MAX_LOOK_BEHIND, Math.floor(value || 0)));
}
function getScanStart(view: EditorView, lookBehind: number, exactScanLimit: number): number {
  ...
  const dynamicLookBehind = Math.max(MIN_LOOK_BEHIND, Math.min(MAX_LOOK_BEHIND, visibleSpan * 3));
  return Math.max(0, firstVisibleFrom - Math.max(lookBehind, dynamicLookBehind));
}
...
function isSkipContext(name: string): boolean {
  ...
  return lower.includes("string") || lower.includes("comment") || lower.includes("regexp") || lower.includes("regex") || lower.includes("regular");
}
function buildTheme(colors: readonly string[]) {
  const themeSpec: Record<string, {
    color: string;
  }> = {};
...
export function getRainbowBracketColors(themeConfig: RainbowBracketThemeConfig = {}): string[] {
  const fallback = themeConfig.dark ? DEFAULT_DARK_COLORS : DEFAULT_LIGHT_COLORS;
  const colors: string[] = [];
  const seen = new Set<string>();
  for (const candidate of [themeConfig.keyword, themeConfig.type, themeConfig.class, themeConfig.function, themeConfig.string, themeConfig.number, themeConfig.constant, themeConfig.variable, themeConfig.foreground]) {
...
export function rainbowBrackets(options: RainbowBracketsOptions = {}) {
  const colors = options.colors != null && options.colors.length > 0 ? [...options.colors] : getRainbowBracketColors();
  const exactScanLimit = Math.max(MIN_LOOK_BEHIND, Math.floor(options.exactScanLimit || DEFAULT_EXACT_SCAN_LIMIT));
  const lookBehind = clampLookBehind(options.lookBehind);
  const theme = buildTheme(colors);
  const marks = colors.map((_, index) => Decoration.mark({
  ...
  const rainbowBracketsPlugin = ViewPlugin.fromClass(class {
    ...
    buildDecorations(view: EditorView): DecorationSet {
      ...
      const isVisible = (pos: number): boolean => {
        while (visibleRangeIndex < visibleRanges.length && pos >= visibleRanges[visibleRangeIndex].to) {
          visibleRangeIndex++;
        }
          ...
          if (name === "(" || name === "[" || name === "{" || name === ")" || name === "]" || name === "}") {
            ...
            if (isOpeningBracket(name)) {
              ...
              for (let index = openBrackets.length - 1; index >= 0; index--) {
                ...
                if (isVisible(pos)) {
                  builder.add(pos, pos + 1, marks[openBrackets[index].colorIndex]);
                }
