  ...
  for (const pos of positions) {
    ...
    for (const assoc of [-1, 1] as const) {
      ...
      for (; node; node = node.parent) {
        ...
        if (node.name === "OpenTag" || node.name === "CloseTag" || node.name === "SelfClosingTag" || node.name === "Element" || node.type.isTop) {
          break;
        }
      }
    }
...
function getRenameTarget(state: EditorState, from: number, to: number): RenameTarget | null {
  const tagName = getTagNameNode(state, from, to);
  const tag = tagName?.parent;
  const element = tag?.parent;
  if (!tagName || !tag || !element || element.name !== "Element" || tag.name !== "OpenTag" && tag.name !== "CloseTag") {
    return null;
  }
...
function createPairedRename(transaction: Transaction): ChangeSpec | null {
  if (!transaction.docChanged || transaction.annotation(skipTagAutoRename) || transaction.annotation(Transaction.remote)) {
    return null;
  }
  const ranges = getChangedRanges(transaction);
  if (ranges.length !== 1) return null;
  const [change] = ranges;
  const target = getRenameTarget(transaction.startState, change.fromA, change.toA);
  if (!target || !touchesRange(change, target.tagName.from, target.tagName.to)) {
    return null;
  }
  if (touchesRange(change, target.pairedTagName.from, target.pairedTagName.to)) {
    return null;
  }
  const editedRange = mapEditedRange(transaction, target.tagName.from, target.tagName.to);
  const newName = transaction.newDoc.sliceString(editedRange.from, editedRange.to);
...
export default function tagAutoRename(): Extension {
  return EditorState.transactionFilter.of(transaction => {
    ...
    return [transaction, {
      ...
      annotations: [skipTagAutoRename.of(true), Transaction.userEvent.of("input.tag-rename")]
    }];
  });
}
