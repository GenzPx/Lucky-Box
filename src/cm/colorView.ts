...
import { COLOR_CHIP_CLASS, colorChipDecoration, colorChipTheme, findColorChip, getColorChipPayload, isViewEditable, openColorPicker, type ColorChipPayload } from "./colorChip";
import { didReplaceLspDocumentColors, lspDocumentColorsField } from "./lsp/documentColors";
interface DocRange {
  from: number;
  to: number;
}
...
function mapRanges(ranges: readonly DocRange[], changes: ViewUpdate["changes"]): DocRange[] {
  return normalizeRanges(ranges.map(({
    from,
    to
  }) => ({
...
function intersectsRanges(from: number, to: number, ranges: readonly DocRange[]): boolean {
  return ranges.some(range => intersectsRange(from, to, range));
}
function intersectRanges(ranges: readonly DocRange[], bounds: readonly DocRange[]): DocRange[] {
  const intersections: DocRange[] = [];
  for (const range of ranges) {
    for (const bound of bounds) {
      const from = Math.max(range.from, bound.from);
      const to = Math.min(range.to, bound.to);
      if (from <= to) intersections.push({
        from,
        to
      });
    }
...
function subtractRanges(ranges: readonly DocRange[], coveredRanges: readonly DocRange[]): DocRange[] {
  ...
  for (const range of normalizeRanges(ranges)) {
    ...
    for (const cover of covered) {
      ...
      if (cover.from > from) {
        result.push({
          from,
          to: Math.min(cover.from, range.to)
        });
      }
      from = Math.max(from, cover.to);
      if (from >= range.to) break;
    }
    if (from < range.to) result.push({
      from,
      to: range.to
    });
  }
...
function colorRanges(view: EditorView, ranges: readonly DocRange[]): Range<Decoration>[] {
...
class ColorViewPlugin {
  ...
  update(update: ViewUpdate): void {
    ...
    if (didReplaceLspDocumentColors(update)) {
      ...
      this.pendingDirtyRanges = normalizeRanges([...this.pendingDirtyRanges, ...update.view.visibleRanges]);
  ...
  scheduleVisibleRanges(view: EditorView): void {
    ...
    this.pendingDirtyRanges = normalizeRanges([...this.pendingDirtyRanges, ...view.visibleRanges]);
    this.scheduleFlush();
  }
  scheduleDecorations(update: ViewUpdate): void {
    ...
    const mappedPreviousVisible = update.docChanged ? mapRanges(this.visibleRanges, update.changes) : this.visibleRanges;
    ...
    this.pendingView = view;
    this.pendingDirtyRanges = normalizeRanges([...this.pendingDirtyRanges, ...dirtyRanges]);
  ...
  flushDecorations(view: EditorView, dirtyRanges: readonly DocRange[]): void {
    ...
    this.decorations = this.decorations.update({
      filter: (from, to) => intersectsRanges(from, to, visibleRanges) && !intersectsRanges(from, to, dirtyVisibleRanges) && !(lspCovered.length && intersectsRanges(from, to, lspCovered)),
      add,
      sort: true
    });
    this.visibleRanges = visibleRanges;
...
async function handleRegexColorPick(view: EditorView, payload: ColorChipPayload): Promise<void> {
...
export const colorView = (showPicker = true) => [colorChipTheme, ViewPlugin.fromClass(ColorViewPlugin, {
