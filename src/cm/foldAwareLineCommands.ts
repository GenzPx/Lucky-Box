import { copyLineDown, copyLineUp, deleteLine, moveLineDown, moveLineUp } from "@codemirror/commands";
import { foldEffect, foldedRanges } from "@codemirror/language";
import { EditorSelection, type EditorState, type SelectionRange, Text } from "@codemirror/state";
...
function firstGroupEndingAtOrAfter(groups: FoldGroup[], position: number) {
  ...
  while (low < high) {
    ...
    if (groups[middle].blockTo < position) low = middle + 1;else high = middle;
  }
  return low;
}
function expandBlockToFolds(block: Pick<LineBlock, "from" | "to">, index: FoldIndex): Pick<LineBlock, "from" | "to"> {
  let from = block.from;
  let to = block.to;
...
function selectionLineBlock(state: EditorState, range: SelectionRange, rangeIndex: number, index: FoldIndex): LineBlock {
...
function selectedLineBlocks(state: EditorState, index: FoldIndex): LineBlock[] {
  const blocks = state.selection.ranges.map((range, rangeIndex) => selectionLineBlock(state, range, rangeIndex, index)).sort((a, b) => a.from - b.from);
  ...
  for (const block of blocks) {
    ...
    if (previous && block.from <= previous.to + 1) {
      ...
      merged.push({
        ...block,
        rangeIndexes: [...block.rangeIndexes]
      });
    }
  }
  return merged;
...
function blockHasFolds(index: FoldIndex, block: Pick<LineBlock, "from" | "to">) {
  const start = firstGroupEndingAtOrAfter(index.groups, block.from);
  return start < index.groups.length && index.groups[start].blockFrom <= block.to;
}
...
function createFoldEffects(folds: FoldRange[], docLength: number) {
  const validFolds = uniqueFolds(folds).filter(fold => fold.from >= 0 && fold.to <= docLength && fold.to > fold.from);
...
function mapRangeIntoBlock(range: SelectionRange, block: LineBlock, newBlockFrom: number) {
  return EditorSelection.range(newBlockFrom + range.anchor - block.from, newBlockFrom + range.head - block.from);
}
function copyFoldedLines(view: EditorView, direction: Direction): boolean {
  ...
  const changes = blocks.map(block => {
    ...
    return copyDown ? {
      from: block.from,
      insert: text.append(lineBreak)
    } : {
      from: block.to,
      insert: lineBreak.append(text)
    };
  });
  const changeSet = state.changes(changes);
  const mappedRanges: SelectionRange[] = new Array(state.selection.ranges.length);
  ...
  for (const block of blocks) {
    ...
    for (const rangeIndex of block.rangeIndexes) {
      mappedRanges[rangeIndex] = mapRangeIntoBlock(state.selection.ranges[rangeIndex], block, selectedBlockFrom);
    }
    for (const fold of foldsInBlock(foldIndex, block)) {
      newFolds.push(translatedFold(fold, block.from, topBlockFrom), translatedFold(fold, block.from, bottomBlockFrom));
    }
  }
  view.dispatch({
    changes: changeSet,
...
function neighboringBlock(state: EditorState, block: LineBlock, direction: Direction, index: FoldIndex): LineBlock | null {
...
function createMoveOperation(state: EditorState, block: LineBlock, neighbor: LineBlock, direction: Direction): MoveOperation {
...
function moveFoldedLines(view: EditorView, direction: Direction): boolean {
  ...
  const operations = blocks.flatMap(block => {
    const neighbor = neighboringBlock(initialState, block, direction, foldIndex);
    return neighbor ? [createMoveOperation(initialState, block, neighbor, direction)] : [];
  });
  if (!operations.length) return false;
  if (!operations.some(operation => blockHasFolds(foldIndex, operation.block) || blockHasFolds(foldIndex, operation.neighbor))) {
    return direction < 0 ? moveLineUp(view) : moveLineDown(view);
  }
  const state = initialState;
  const changeSet = state.changes(operations.map(operation => ({
    from: operation.replaceFrom,
    to: operation.replaceTo,
    insert: operation.insert
  })));
  const mappedRanges = state.selection.ranges.map(range => EditorSelection.range(changeSet.mapPos(range.anchor), changeSet.mapPos(range.head)));
  const newFolds: FoldRange[] = [];
  for (const operation of operations) {
    ...
    for (const rangeIndex of operation.block.rangeIndexes) {
      mappedRanges[rangeIndex] = mapRangeIntoBlock(state.selection.ranges[rangeIndex], operation.block, newBlockFrom);
    }
    ...
    for (const fold of foldsInBlock(foldIndex, operation.neighbor)) {
      newFolds.push(translatedFold(fold, operation.neighbor.from, newNeighborFrom));
    }
  }
  view.dispatch({
    changes: changeSet,
...
export function deleteLineFoldAware(view: EditorView): boolean {
  ...
  const changeSet = state.changes(changes);
  const cursors = blocks.map(block => EditorSelection.cursor(Math.min(changeSet.newLength, changeSet.mapPos(block.from, -1))));
  view.dispatch({
    changes: changeSet,
