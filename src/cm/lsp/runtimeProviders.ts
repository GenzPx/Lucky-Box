import type { LspRuntimeContext, LspRuntimeProvider, LspServerDefinition, WorkspaceKind } from "./types";
...
function normalizeProvider(provider: LspRuntimeProvider): LspRuntimeProvider {
  ...
  for (const method of ["resolveUris", "checkInstallation", "install", "uninstall", "getInstallCommand", "getUninstallCommand", "stop"] as const) {
...
function canProviderRunServer(provider: LspRuntimeProvider, server: LspServerDefinition): boolean {
  const allowed = getAllowedRuntimes(server);
  if (allowed) return allowed.has(provider.id);
  return true;
}
function withDerivedContext(server: LspServerDefinition, context: LspRuntimeContext = {}): LspRuntimeContext {
  return {
    ...context,
...
export function registerRuntimeProvider(provider: LspRuntimeProvider, options: RegisterRuntimeProviderOptions = {}): LspRuntimeProvider {
  const normalized = normalizeProvider(provider);
  if (providers.has(normalized.id) && !options.replace) {
    throw new Error(`LSP runtime provider ${normalized.id} is already registered`);
  }
...
export function listRuntimeProviders(): LspRuntimeProvider[] {
  return Array.from(providers.values()).sort((a, b) => getPriority(b) - getPriority(a) || a.id.localeCompare(b.id));
}
export async function selectRuntimeProvider(server: LspServerDefinition, context: LspRuntimeContext = {}): Promise<LspRuntimeProvider | null> {
  ...
  if (configuredRuntimeId) {
    ...
    if (!configuredProvider) {
      console.warn(`Configured LSP runtime provider ${configuredRuntimeId} is not registered`);
    } else if (!canProviderRunServer(configuredProvider, server)) {
      console.warn(`Configured LSP runtime provider ${configuredRuntimeId} is not allowed for ${server.id}`);
    } else {
      try {
        if (await configuredProvider.canHandle(server, runtimeContext)) {
          return configuredProvider;
        }
        console.warn(`Configured LSP runtime provider ${configuredRuntimeId} cannot handle ${server.id}`);
      } catch (error) {
        console.warn(`Configured LSP runtime provider ${configuredRuntimeId} failed canHandle() for ${server.id}`, error);
      }
    }
  }
  for (const provider of listRuntimeProviders()) {
    ...
    try {
      ...
      console.warn(`LSP runtime provider ${provider.id} failed canHandle() for ${server.id}`, error);
    }
  }
  return null;
}
export function inferWorkspaceKind(context: Pick<LspRuntimeContext, "uri" | "rootUri" | "file">): WorkspaceKind {
  ...
  if (scheme === "ftp" || scheme === "sftp" || scheme === "http" || scheme === "https") {
    return "remote";
  }
...
export function isBuiltinAlpineAccessible(context: Pick<LspRuntimeContext, "uri" | "rootUri" | "file">): boolean {
  const uri = String(context.rootUri || context.file?.uri || context.uri || "");
  if (!uri) return false;
  const schemeMatch = /^([a-zA-Z][\w+\-.]*?):/.exec(uri);
  const scheme = schemeMatch ? schemeMatch[1].toLowerCase() : null;
