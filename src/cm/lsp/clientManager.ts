...
import type { LSPClientExtension, LSPPluginOptions } from "@codemirror/lsp-client";
import { LSPClient, LSPPlugin, serverCompletion, serverDiagnostics } from "@codemirror/lsp-client";
...
import { clearDiagnosticsEffect, disposePullDiagnostics, lspDiagnosticsAutoSyncExtension } from "./diagnostics";
...
import { hoverTooltips, resolveLspHoverHighlightLanguage, signatureHelp } from "./tooltipExtensions";
import { createTransport } from "./transport";
import type { BuiltinExtensionsConfig, ClientManagerOptions, ClientState, DocumentUriContext, FileMetadata, FormattingOptions, LspServerDefinition, LspRuntimeConnection, LspRuntimeProvider, LspClientScope, NormalizedRootUri, ParsedUri, RootUriContext, TextEdit, TransportContext, TransportHandle, Transport } from "./types";
...
function pluginKey(serverId: string, rootUri: string | null | undefined, useWorkspaceFolders?: boolean): string {
...
function formatInitializationError(error: unknown): string {
  ...
  const message = error instanceof Error ? error.message : safeString(error).trim();
  return message ? `Initialization failed: ${message}` : "Initialization failed";
}
function isSettingsOrKeybindingsFile(uri: string | null | undefined, file?: {
  uri?: string;
} | null): boolean {
  ...
  try {
    ...
    if (dataStorage) {
      const settingsPath = String(Url.join(dataStorage, "settings.json")).toLowerCase();
      const keybindingsPath = String((globalThis as any).KEYBINDING_FILE || Url.join(dataStorage, ".key-bindings.json")).toLowerCase();
      if (fileUri === settingsPath || fileUri === keybindingsPath) {
        return true;
      }
    }
  } catch {}
  return fileUri.endsWith("/settings.json") || fileUri.endsWith("/.key-bindings.json") || fileUri.endsWith("/keybindings.json") || fileUri.endsWith("/.keybindings.json") || fileUri === "settings.json" || fileUri === ".key-bindings.json" || fileUri === "keybindings.json" || fileUri === ".keybindings.json";
}
function isVerboseLspLoggingEnabled(): boolean {
  const buildInfo = (globalThis as {
    BuildInfo?: {
      debug?: boolean;
    };
  }).BuildInfo;
...
function resolveInitializationOptions(server: LspServerDefinition, clientConfig: Record<string, unknown>): Record<string, unknown> | undefined {
  const serverOptions = isPlainObject(server.initializationOptions) ? server.initializationOptions : null;
  const clientOptions = isPlainObject(clientConfig.initializationOptions) ? clientConfig.initializationOptions : null;
...
type RequestInnerFn = <Params, Result>(method: string, params: Params, mapped?: boolean) => InternalLSPRequest<Result>;
function connectClient(client: ExtendedLSPClient, transport: Transport, initializationOptions?: Record<string, unknown>, rootUri?: string | null): void {
  const hasInitializationOptions = !!initializationOptions && Object.keys(initializationOptions).length > 0;
  ...
  const originalRequestInner = patchedClient.requestInner.bind(patchedClient) as RequestInnerFn;
  patchedClient.requestInner = function patchedRequestInner<Params, Result>(method: string, params: Params, mapped?: boolean): InternalLSPRequest<Result> {
...
function buildBuiltinExtensions(config: BuiltinExtensionsConfig = {}): BuiltinExtensionsResult {
  const {
...
function throwIfInitializationCancelled(signal: AbortSignal, serverId: string): void {
  if (!signal.aborted) return;
  throw initializationCancelledError(serverId);
}
function waitForInitialization<T>(promise: PromiseLike<T>, signal: AbortSignal, serverId: string): Promise<T> {
...
export class LspClientManager {
  ...
  async getExtensionsForFile(metadata: FileMetadata): Promise<Extension[]> {
    ...
    for (const server of servers) {
      ...
      if (!target) {
        console.warn(`Cannot resolve runtime or document URI for LSP server ${server.id}: ${originalUri}`);
        continue;
      }
      ...
      if (server.resolveLanguageId) {
        try {
          ...
          console.warn(`LSP server ${server.id} failed to resolve language id for ${normalizedUri}`, error);
        }
      }
      try {
        ...
        const plugin = clientState.client.plugin(normalizedUri, targetLanguageId, buildPluginOptions(server));
        if (server.clientConfig?.builtinExtensions?.completion !== false) {
          lspExtensions.push(lspCompletionEnabled.of(true));
        }
        const aliases = originalUri && originalUri !== normalizedUri ? [originalUri] : [];
        clientState.attach(normalizedUri, view as EditorView, aliases);
        lspExtensions.push(plugin);
      } catch (error) {
        console.error(`Failed to initialize LSP client for ${server.id}`, error);
      }
    }
  ...
  async formatDocument(metadata: FileMetadata, options: FormattingOptions = {}): Promise<boolean> {
    const {
      uri: originalUri,
      languageId,
      languageName,
    ...
    for (const server of servers) {
      ...
      try {
        ...
        if (!target) {
          console.warn(`Cannot resolve document URI for formatting with ${server.id}: ${originalUri}`);
          continue;
        }
        const normalizedUri = target.documentUri;
        ...
        }, TextEdit[] | null>("textDocument/formatting", {
          textDocument: {
            uri: normalizedUri
          },
  ...
  async disposeServer(serverId: string): Promise<void> {
    ...
    for (const [key, pending] of this.#pendingClients.entries()) {
      ...
      pendingDisposals.push(pending.promise.then(state => state.dispose()));
    }
    const states = Array.from(this.#clients.values()).filter(state => state.server.id.toLowerCase() === normalizedId);
    await Promise.allSettled([...states.map(state => state.dispose()), ...pendingDisposals]);
  }
  async dispose(): Promise<void> {
    try {
      ...
      const em = (globalThis as Record<string, unknown>).editorManager as EditorManagerLike | undefined;
      if (em?.editor) {
        try {
          em.editor.dispatch({
      ...
      if (em?.files) {
        for (const file of em.files) {
          if (file?.type !== "editor" || file.id === em.activeFile?.id) continue;
  ...
  async #ensureClient(server: LspServerDefinition, context: RootUriContext, target: ResolvedRuntimeTarget): Promise<ClientState> {
    const {
      documentUri,
      normalizedRootUri,
      originalRootUri,
    ...
    const useWsFolders = scope === "workspace" && server.useWorkspaceFolders === true;
    const runtimeServerKey = `${server.id}@${runtimeProvider.id}`;
    const key = scope === "document" ? `${runtimeServerKey}::__document__::${documentUri}` : pluginKey(runtimeServerKey, normalizedRootUri, useWsFolders);
  ...
  async #initializeClient(server: LspServerDefinition, context: RootUriContext, initContext: InitContext): Promise<ClientState> {
    const {
      key,
      normalizedRootUri,
      originalRootUri,
    ...
    const initializationOptions = resolveInitializationOptions(server, clientConfig as Record<string, unknown>);
    ...
    const {
      ...
      diagnosticsExtension
    } = useDefaultExtensions ? buildBuiltinExtensions({
    ...
    const wantsCustomDiagnostics = [...extraExtensions, ...serverExtensions].some(ext => {
      const extWithCaps = ext as ExtensionWithCapabilities;
      return !!extWithCaps?.clientCapabilities?.textDocument?.publishDiagnostics;
    });
    const filteredBuiltins = wantsCustomDiagnostics && diagnosticsExtension ? defaultExtensions.filter(ext => ext !== diagnosticsExtension) : defaultExtensions;
    ...
    const mergedExtensions = [...filteredBuiltins, ...extraExtensions, ...serverExtensions, clientCapabilities];
    ...
    clientConfig.notificationHandlers = {
      ...
      "$/progress": (_client: LSPClient, params: unknown): boolean => {
        ...
        logLspInfo(`[LSP:${server.id}] Progress: ${kind} - ${message || title || ""} ${typeof percentage === "number" ? `(${percentage}%)` : ""}`);
        return true;
      },
      "$/typescriptVersion": (_client: LSPClient, params: unknown): boolean => {
        ...
        logLspInfo(`[LSP:${server.id}] TypeScript ${versionParams.version} (${source})`);
        lspStatusBar.show({
    ...
    const unhandledNotificationKey = "unhandledNotification" as keyof typeof clientConfig;
    if (!(unhandledNotificationKey in clientConfig)) {
      (clientConfig as Record<string, (client: LSPClient, method: string, params: unknown) => void>).unhandledNotification = (_client: LSPClient, method: string, params: unknown) => {
        logLspInfo(`[LSP:${server.id}] Unhandled notification: ${method}`, params);
      };
    }
    if (!clientConfig.workspace) {
      clientConfig.workspace = (client: LSPClient) => new AcodeWorkspace(client, workspaceOptions);
    }
    ...
    try {
      ...
      const runtimeContext = {
        ...
        serverId: server.id,
        allowNonTerminalWorkspace: this.options.allowNonTerminalWorkspace === true
      };
      ...
      throwIfInitializationCancelled(signal, server.id);
      transportHandle = createTransportFromRuntimeConnection(server, runtimeContext, connection);
      ...
      connectClient(client, transportHandle.transport, initializationOptions, scope === "workspace" && server.useWorkspaceFolders ? null : normalizedRootUri);
      ...
      if (!client.__acodeLoggedInfo) {
        if (normalizedRootUri) {
          if (originalRootUri && originalRootUri !== normalizedRootUri) {
            logLspInfo(`[LSP:${server.id}] root ${normalizedRootUri} (from ${originalRootUri})`);
          } else {
        ...
        if (initializationOptions) {
          logLspInfo(`[LSP:${server.id}] initializationOptions keys`, Object.keys(initializationOptions));
        }
        logLspInfo(`[LSP:${server.id}] initialized`);
        addLspLog(server.id, "info", normalizedRootUri ? `Initialized workspace ${normalizedRootUri}` : "Initialized without a workspace root");
        client.__acodeLoggedInfo = true;
      }
    } catch (error) {
      addLspLog(server.id, "error", formatInitializationError(error), error);
      try {
        client?.disconnect();
      } catch {}
      if (transportHandle) {
  ...
  #createClientState(params: {
    ...
    let disposed = false;
    const attach = (uri: string, view: EditorView, aliases: string[] = []): void => {
    ...
    const clearClientDiagnostics = (view: EditorView): void => {
      try {
        view.dispatch({
          effects: clearDiagnosticsEffect(client)
        });
      } catch {}
    };
  ...
  async #resolveRootUri(server: LspServerDefinition, context: RootUriContext): Promise<string | null> {
  ...
  async #resolveRuntimeTarget(server: LspServerDefinition, context: RootUriContext): Promise<ResolvedRuntimeTarget | null> {
    ...
    const {
      normalizedRootUri
    } = normalizeRootUriForServer(server, originalRootUri);
    const normalizedDocumentUri = await this.#resolveDocumentUri(server, context);
    ...
    if (!runtimeProvider) {
      console.warn(`No LSP runtime provider selected for ${server.id}: uri=${originalDocumentUri}, root=${originalRootUri ?? "none"}, normalizedUri=${normalizedDocumentUri ?? "none"}`);
      return null;
    }
    ...
    if (runtimeProvider.resolveUris) {
      try {
        ...
        console.warn(`LSP runtime provider ${runtimeProvider.id} failed to resolve URIs for ${server.id}`, error);
        return null;
      }
    }
    if (!documentUri) {
      console.warn(`LSP runtime provider ${runtimeProvider.id} produced no document URI for ${server.id}: uri=${originalDocumentUri}, normalizedUri=${normalizedDocumentUri ?? "none"}`);
      return null;
    }
    return {
      originalDocumentUri,
  ...
  async #resolveDocumentUri(server: LspServerDefinition, context: RootUriContext): Promise<string | null> {
    ...
    if (typeof server.documentUri === "function") {
      try {
        ...
        console.warn(`Server document URI resolver failed for ${server.id}`, error);
      }
    }
    return normalizedUri;
  }
}
function createTransportFromRuntimeConnection(server: LspServerDefinition, context: TransportContext, connection: LspRuntimeConnection): TransportHandle {
...
function applyTextEdits(plugin: LSPPlugin, view: EditorView, edits: TextEdit[]): boolean {
  ...
  for (const edit of edits) {
    ...
    const fromResult = plugin.unsyncedChanges.mapPos(fromBase, 1, MapMode.TrackDel);
    const toResult = plugin.unsyncedChanges.mapPos(toBase, -1, MapMode.TrackDel);
    if (fromResult == null || toResult == null) continue;
    const insert = typeof edit.newText === "string" ? edit.newText.replace(/\r\n/g, "\n") : "";
    changes.push({
      from: fromResult,
      to: toResult,
      insert
...
function buildFormattingOptions(view: EditorView, overrides: FormattingOptions = {}): FormattingOptions {
  const state = view?.state;
  if (!state) return {
    ...overrides
  };
  const unitValue = state.facet(indentUnit);
  const unit = typeof unitValue === "string" && unitValue.length ? unitValue : String(unitValue ?? "\t");
...
function normalizeRootUriForServer(_server: LspServerDefinition, rootUri: string | null): NormalizedRootUri {
...
function contentUriToFileUri(uri: string): string | null {
  try {
    ...
    const providerMatch = /^content:\/\/com\.((?![:<>"/\\|?*]).*?)\.documents\//.exec(rootUri ?? "");
