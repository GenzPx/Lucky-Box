...
import { checkServerInstallation, ensureServerRunning, getInstallCommand as getAlpineInstallCommand, getUninstallCommand as getAlpineUninstallCommand, installServer, uninstallServer } from "../serverLauncher";
import { isBuiltinAlpineAccessible } from "../runtimeProviders";
import type { LspRuntimeContext, LspRuntimeProvider, LspServerDefinition, LspRuntimeUriResolutionContext } from "../types";
export const BUILTIN_ALPINE_RUNTIME_ID = "builtin-alpine";
function isUntitled(context: LspRuntimeContext): boolean {
  return /^untitled:/i.test(String(context.originalDocumentUri || context.uri || ""));
}
...
export const builtinAlpineRuntimeProvider: LspRuntimeProvider = {
  ...
  priority: -100,
  canHandle(server: LspServerDefinition, context: LspRuntimeContext): boolean {
    if (context.runtimeAction && server.launcher) {
      return true;
    }
    return !!server.launcher && (canUseRealPath(context) || isUntitled(context) && !!cacheDocumentUri(context) || context.allowNonTerminalWorkspace === true && !!cacheDocumentUri(context));
  },
  resolveUris(server: LspServerDefinition, context: LspRuntimeUriResolutionContext) {
    if (canUseRealPath(context) && !isUntitled(context)) return null;
    const documentUri = cacheDocumentUri(context);
    if (!documentUri) {
      throw new Error(`Built-in Alpine cannot resolve a cache URI for ${context.originalDocumentUri}`);
    }
    return {
      documentUri,
      rootUri: null,
  ...
  async install(server, context, mode, options) {
    ...
    if (!isTerminalInstalled) {
      const message = strings?.terminal_required_message_for_lsp ?? "Terminal not installed. Please install Terminal first to use LSP servers.";
