...
import { openReferencesTab, showReferencesPanel } from "components/referencesPanel";
  ...
  try {
    ...
    if (openFile?.session?.doc) {
      ...
      if (typeof doc.toString === "function") {
        const content = getDocText(doc as {
          toString(): string;
        });
    ...
    if (fs && (await fs.exists())) {
      const encoding = (settings as {
        value?: {
          defaultFileEncoding?: string;
        };
      })?.value?.defaultFileEncoding || "utf-8";
...
async function fetchReferences(view: EditorView): Promise<{
  symbolName: string;
  references: ReferenceWithContext[];
} | null> {
  const plugins = LSPPlugin.getAll(view, "references").filter(plugin => !!plugin.client.serverCapabilities?.referencesProvider);
  if (!plugins.length) {
    const toast = (globalThis as Record<string, unknown>).toast as ((msg: string) => void) | undefined;
  ...
  const settled = await Promise.allSettled(plugins.map(async plugin => {
    plugin.client.sync();
    return (await plugin.client.request<ReferenceParams, Location[] | null>("textDocument/references", {
      textDocument: {
        uri: plugin.uri
      },
      position: {
  ...
  const refsWithContext: ReferenceWithContext[] = await Promise.all(locations.map(async loc => {
...
export async function findAllReferences(view: EditorView): Promise<boolean> {
  ...
  const symbolName = getWordAtCursor(view);
  const panel = showReferencesPanel({
    symbolName
  });
  try {
...
export async function findAllReferencesInTab(view: EditorView): Promise<boolean> {
  const plugin = LSPPlugin.get(view);
  if (!plugin) {
    const toast = (globalThis as Record<string, unknown>).toast as ((msg: string) => void) | undefined;
...
export function closeReferencesPanel(): boolean {
  const {
    hideReferencesPanel
  } = require("components/referencesPanel");
  hideReferencesPanel();
  return true;
}
