...
import { forgetPullDiagnostics, schedulePullDiagnostics } from "./diagnostics";
...
export default class AcodeWorkspace extends Workspace {
  ...
  constructor(client: ConstructorParameters<typeof Workspace>[0], options: WorkspaceOptions = {}) {
    super(client);
    this.files = [];
  ...
  #getOrCreateFile(uri: string, languageId: string, view: EditorView): AcodeWorkspaceFile {
    let file = this.#fileMap.get(uri);
    if (!file) {
      const doc = view.state?.doc;
      if (!doc) {
        throw new Error(`Cannot create workspace file without document: ${uri}`);
      }
      file = new AcodeWorkspaceFile(uri, languageId, this.#nextFileVersion(uri), doc, view);
  ...
  #resolveLanguageIdForUri(uri: string): string {
    ...
    try {
      ...
      this.#log("warn", `Workspace failed to resolve language id for ${uri}`, error);
      console.warn(`[LSP:Workspace] Failed to resolve language id for ${uri}`, error);
    }
    return "plaintext";
  }
  ...
  async #applyUpdateToClosedFile(uri: string, update: TransactionSpec): Promise<void> {
