...
import type { InlayHint, InlayHintLabelPart, Position } from "vscode-languageserver-types";
...
class HintWidget extends WidgetType {
  constructor(readonly label: string, readonly padLeft: boolean, readonly padRight: boolean, readonly tooltip: string | undefined) {
    super();
  }
  eq(other: HintWidget): boolean {
    return this.label === other.label && this.padLeft === other.padLeft && this.padRight === other.padRight;
  }
...
function buildDecos(hints: ProcessedHint[], docLen: number): DecorationSet {
  ...
  for (const h of hints) {
    ...
    decos.push(Decoration.widget({
      widget: new HintWidget(h.label, h.paddingLeft ?? false, h.paddingRight ?? false, h.tooltip),
      side: 1
    }).range(h.pos));
  }
...
function createPlugin(config: InlayHintsConfig) {
  ...
  return ViewPlugin.fromClass(class {
    ...
    update(update: ViewUpdate): void {
      if (update.transactions.some(t => t.effects.some(e => e.is(setHints)))) {
        this.decorations = buildDecos(update.state.field(hintsField, false) ?? [], update.state.doc.length);
      }
    ...
    async fetch(): Promise<void> {
      const providers = (LSPPlugin.getAll(this.view, "inlayHint") as readonly LSPPluginAPI[]).filter(lsp => lsp.client.connected && !!lsp.client.serverCapabilities?.inlayHintProvider);
      ...
      const endLn = Math.min(doc.lines, doc.lineAt(Math.min(doc.length, to)).number + buf);
      const settled = await Promise.allSettled(providers.map(async lsp => {
        lsp.client.sync();
        const hints = await lsp.client.request<InlayHintParams, InlayHint[] | null>("textDocument/inlayHint", {
          textDocument: {
            uri: lsp.uri
          },
          range: {
    ...
    process(lsp: LSPPluginAPI, hints: InlayHint[], docLen: number): ProcessedHint[] {
      ...
      for (const h of hints) {
        ...
        const label = typeof h.label === "string" ? h.label : Array.isArray(h.label) ? h.label.map((p: InlayHintLabelPart) => p.value).join("") : "";
        if (!label) continue;
        const tooltip = typeof h.tooltip === "string" ? h.tooltip : h.tooltip && typeof h.tooltip === "object" && "value" in h.tooltip ? (h.tooltip as {
          value: string;
        }).value : undefined;
        result.push({
          pos,
...
export function inlayHintsEditorExtension(config: InlayHintsConfig = {}): Extension {
  if (config.enabled === false) return [];
  if (!Object.keys(config).length) {
    defaultInlayHintsEditorExtension ??= [hintsField, createPlugin({}), styles];
    return defaultInlayHintsEditorExtension;
  }
  return [hintsField, createPlugin(config), styles];
}
export function inlayHintsExtension(config: InlayHintsConfig = {}): LSPClientExtension & {
