...
import type { Color, ColorInformation, ColorPresentation, Range as LspRange, TextEdit } from "vscode-languageserver-types";
import { clamp01, colorChipDecoration, colorChipTheme, cssToLspColor, detectColorFormat, findColorChip, getColorChipPayload, hasLspColorProvider, isViewEditable, lspColorToCss, lspColorToHex, openColorPicker, type ColorChipPayload } from "../colorChip";
...
function pickPresentation(presentations: ColorPresentation[], originalText: string, pickedCss: string): ColorPresentation | null {
...
function viewportBounds(view: EditorView, bufferLines: number): {
  from: number;
  to: number;
} {
...
function buildDecos(colors: readonly LspDocumentColor[], bounds: {
  from: number;
  to: number;
}, docLen: number): DecorationSet {
...
function mapLspRange(lsp: LSPPluginAPI, range: LspRange, docLen: number): {
  from: number;
  to: number;
} | null {
  let from: number;
  ...
  try {
    ...
    const fromMapped = lsp.unsyncedChanges.mapPos(fromBase, 1, MapMode.TrackDel);
...
function applyTextEdits(lsp: LSPPluginAPI, view: EditorView, edits: TextEdit[]): boolean {
  const changes: {
    from: number;
    to: number;
    insert: string;
...
export function didReplaceLspDocumentColors(update: ViewUpdate): boolean {
  return update.transactions.some(t => t.effects.some(e => e.is(setColors)));
}
function createPlugin(config: DocumentColorsConfig) {
  ...
  return ViewPlugin.fromClass(class {
    ...
    update(update: ViewUpdate): void {
      ...
      if (update.viewportChanged || update.docChanged || didReplaceLspDocumentColors(update)) {
        this.decorations = buildDecos(colors, viewportBounds(update.view, bufferLines), update.state.doc.length);
      }
    ...
    async fetch(): Promise<void> {
      ...
      const bindings = LSPPlugin.getAll(this.view, "documentColor") as readonly LSPPluginAPI[];
      ...
      this.capRetries = 0;
      const providers = connected.filter(lsp => !!lsp.client.serverCapabilities?.colorProvider);
      ...
      const id = ++this.reqId;
      const settled = await Promise.allSettled(providers.map(async lsp => {
        lsp.client.sync();
        const result = await lsp.client.request<DocumentColorParams, ColorInformation[] | null>("textDocument/documentColor", {
          textDocument: {
            uri: lsp.uri
          }
        });
        return this.process(lsp, result ?? [], this.view.state.doc.length);
      }));
      ...
      this.view.dispatch({
        effects: setColors.of(stored)
      });
    }
    process(lsp: LSPPluginAPI, infos: ColorInformation[], docLen: number): LspDocumentColor[] {
    ...
    clearColors(): void {
      const current = this.view.state.field(lspDocumentColorsField, false) ?? [];
      if (current.length) {
        this.view.dispatch({
          effects: setColors.of([])
        });
      }
    }
    destroy(): void {
...
async function handleColorPick(view: EditorView, payload: ColorChipPayload): Promise<void> {
  const lsp = (payload.lspClient ? LSPPlugin.get(view, payload.lspClient) : LSPPlugin.getForFeature(view, "documentColor")) as LSPPluginAPI | null;
  if (!lsp?.client.connected) return;
  const doc = view.state.doc;
  const stillValid = payload.from >= 0 && payload.to <= doc.length && payload.from < payload.to;
  const currentText = stillValid ? doc.sliceString(payload.from, payload.to) : payload.css;
  const seed = payload.pickerSeed || currentText || payload.css || "#ffffff";
  const picked = await openColorPicker(seed);
  if (!picked) return;
  const lsp2 = (payload.lspClient ? LSPPlugin.get(view, payload.lspClient) : LSPPlugin.getForFeature(view, "documentColor")) as LSPPluginAPI | null;
  ...
  try {
    ...
    const presentations = await lsp2.client.request<ColorPresentationParams, ColorPresentation[] | null>("textDocument/colorPresentation", {
      textDocument: {
        uri: lsp2.uri
      },
      color: lspColor,
...
function findLiveColor(view: EditorView, hint: {
  from: number;
  to: number;
}, expectedText?: string): {
  from: number;
  ...
  const doc = view.state.doc;
  const match = colors.find(c => c.from === hint.from && c.to === hint.to) ?? colors.find(c => c.from <= hint.to && c.to >= hint.from) ?? null;
...
export function documentColorsEditorExtension(config: DocumentColorsConfig = {}): Extension {
  if (config.enabled === false) return [];
  if (!Object.keys(config).length) {
    defaultDocumentColorsEditorExtension ??= [lspDocumentColorsField, createPlugin({}), colorChipTheme];
    return defaultDocumentColorsEditorExtension;
  }
  return [lspDocumentColorsField, createPlugin(config), colorChipTheme];
}
export function documentColorsExtension(config: DocumentColorsConfig = {}): LSPClientExtension & {
