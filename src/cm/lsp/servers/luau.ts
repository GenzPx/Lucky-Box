...
import { quoteArg, runForegroundCommand, runQuickCommand } from "../installRuntime";
import { defineBundle, defineServer, installers } from "../providerUtils";
import type { InstallCheckResult, LspServerBundle, LspServerManifest } from "../types";
function isGlibcRuntimeError(output: string): boolean {
  return output.includes("ld-linux-aarch64.so.1") || output.includes("ld-linux-x86-64.so.2") || output.includes("Error loading shared library") || output.includes("__fprintf_chk") || output.includes("__snprintf_chk") || output.includes("__vsnprintf_chk") || output.includes("__libc_single_threaded") || output.includes("GLIBC_");
}
...
async function readLuauRuntimeFailure(binaryPath: string): Promise<string> {
  ...
  try {
    ...
    try {
      const lddOutput = await runQuickCommand(`command -v ldd >/dev/null 2>&1 && ldd ${quoteArg(binaryPath)} 2>&1 || true`);
...
export const luauServers: LspServerManifest[] = [defineServer({
  id: "luau",
  label: "Luau",
...
export const luauBundle: LspServerBundle = defineBundle({
  ...
  servers: luauServers,
  hooks: {
    getExecutable: (_, manifest) => manifest.launcher?.install?.binaryPath || manifest.launcher?.install?.executable || null,
    async checkInstallation(_, manifest): Promise<InstallCheckResult> {
      const binary = manifest.launcher?.install?.binaryPath || manifest.launcher?.install?.executable;
      if (!binary) {
        return {
          status: "failed",
          version: null,
    ...
    async installServer(_, manifest, mode, options = {}): Promise<boolean> {
      ...
      if (promptConfirm) {
        const shouldContinue = await confirm(label, `${actionLabel} ${label} language server?`);
        if (!shouldContinue) {
          return false;
        }
      }
...
\t*) echo "Unsupported architecture: $ARCH" >&2; exit 1 ;;
      ...
      const loadingDialog = loader.create(label, `${actionLabel}ing ${label}...`);
      try {
        loadingDialog.show();
