import { bindServerRegistry, ensureBuiltinBundlesRegistered } from "./serverCatalog";
import type { AcodeClientConfig, BridgeConfig, LanguageResolverContext, LauncherConfig, LspServerDefinition, LspServerManifest, RegistryEventListener, RegistryEventType, RootUriContext, TransportDescriptor, WebSocketTransportOptions } from "./types";
...
function sanitizeLanguages(languages: string[] = []): string[] {
  ...
  return languages.map(lang => String(lang ?? "").trim().toLowerCase()).filter(Boolean);
}
function sanitizeRuntimeIds(runtimes: unknown): string[] | undefined {
  if (!Array.isArray(runtimes)) return undefined;
  const ids = runtimes.map(runtime => String(runtime ?? "").trim().toLowerCase()).filter(Boolean);
...
function sanitizeInstallKind(value: unknown): "apk" | "npm" | "pip" | "cargo" | "github-release" | "manual" | "shell" | undefined {
  switch (value) {
    case "apk":
    case "npm":
    case "pip":
...
function sanitizeBridge(serverId: string, bridge: RawBridgeConfig | undefined | null): BridgeConfig | undefined {
  if (!bridge || typeof bridge !== "object") return undefined;
  const kind = bridge.kind ?? "axs";
  if (kind !== "axs") {
    throw new Error(`LSP server ${serverId} declares unsupported bridge kind ${kind}`);
  }
  ...
  const args = Array.isArray(bridge.args) ? bridge.args.map(arg => String(arg)) : undefined;
  return {
    kind: "axs",
    port,
    command,
...
function sanitizeDefinition(definition: RawServerDefinition): LspServerDefinition {
  ...
  const kind = (transport.kind ?? "stdio") as "stdio" | "websocket" | "external";
  if (!transport || typeof transport !== "object") {
    throw new Error(`LSP server ${id} is missing a transport descriptor`);
  }
  if (!("languages" in definition) || !sanitizeLanguages(definition.languages).length) {
  ...
  const transportOptions: Record<string, unknown> = transport.options && typeof transport.options === "object" ? {
    ...transport.options
  } : {};
  const sanitizedTransport: TransportDescriptor = {
    kind,
    command: transport.command,
    args: Array.isArray(transport.args) ? transport.args.map(arg => String(arg)) : undefined,
  ...
  if (definition.launcher && typeof definition.launcher === "object") {
    ...
    const installExecutable = typeof rawLauncher.install?.executable === "string" ? rawLauncher.install.executable.trim() : "";
    launcher = {
      command: rawLauncher.command,
      args: Array.isArray(rawLauncher.args) ? rawLauncher.args.map(arg => String(arg)) : undefined,
      startCommand: Array.isArray(rawLauncher.startCommand) ? rawLauncher.startCommand.map(arg => String(arg)) : rawLauncher.startCommand,
      ...
      logOutput: rawLauncher.logOutput === "warnings-and-errors" ? "warnings-and-errors" : "all",
      install: rawLauncher.install && typeof rawLauncher.install === "object" ? {
        ...
        packages: Array.isArray(rawLauncher.install.packages) ? rawLauncher.install.packages.map(entry => String(entry)) : undefined,
        ...
        assetNames: rawLauncher.install.assetNames && typeof rawLauncher.install.assetNames === "object" ? Object.fromEntries(Object.entries(rawLauncher.install.assetNames).map(([key, value]) => [String(key), String(value)])) : undefined,
        archiveType: rawLauncher.install.archiveType === "binary" ? "binary" : "zip",
    ...
    if (isManagedInstall) {
      const providedExecutable = launcher.install?.binaryPath || launcher.install?.executable;
      if (!providedExecutable) {
        throw new Error(`LSP server ${id} managed installers must declare install.binaryPath or install.executable`);
      }
    }
  }
  const sanitized: LspServerDefinition = {
    ...
    priority: typeof definition.priority === "number" && Number.isFinite(definition.priority) ? definition.priority : 0,
    ...
    startupTimeout: typeof definition.startupTimeout === "number" ? definition.startupTimeout : undefined,
    capabilityOverrides: clone(definition.capabilityOverrides),
    rootUri: typeof definition.rootUri === "function" ? definition.rootUri : null,
    documentUri: typeof definition.documentUri === "function" ? definition.documentUri : null,
    resolveLanguageId: typeof definition.resolveLanguageId === "function" ? definition.resolveLanguageId : null,
    launcher,
...
export function registerServer(definition: RawServerDefinition, options: RegisterServerOptions = {}): LspServerDefinition {
  const {
    replace = false
  } = options;
...
export type ServerUpdater = (current: LspServerDefinition) => Partial<LspServerDefinition> | null;
export function updateServer(id: string, updater: ServerUpdater): LspServerDefinition | null {
  const key = toKey(id);
...
export function getServersForLanguage(languageId: string, options: GetServersOptions = {}): LspServerDefinition[] {
  const {
