...
function getActiveMetadata(manager: EditorManagerWithLsp | undefined, file: AcodeFile | undefined): (FileMetadata & {
  view?: EditorView;
}) | null {
...
export function registerLspFormatter(acode: AcodeApi): void {
  ...
  const extensions = languages.size ? collectFormatterExtensions(languages) : ["*"];
  acode.registerFormatter("lsp", extensions, async () => {
    ...
    const servers = serverRegistry.getServersForLanguage(languageId).filter(supportsBuiltinFormatting);
    if (!servers.length) {
...
function collectFormatterExtensions(languages: Set<string>): string[] {
  ...
  try {
    ...
    modes.forEach(mode => {
      const key = String(mode?.name ?? "").trim().toLowerCase();
  ...
  languages.forEach(language => {
    ...
    String(mode.extensions).split("|").forEach(part => {
      const ext = part.trim();
