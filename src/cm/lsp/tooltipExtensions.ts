import { highlightingFor, type Language, LanguageDescription, language as languageFacet } from "@codemirror/language";
import { languages } from "@codemirror/language-data";
import type { LSPClient } from "@codemirror/lsp-client";
import { LSPPlugin } from "@codemirror/lsp-client";
import { type Extension, Prec, StateEffect, StateField } from "@codemirror/state";
import { type Command, closeHoverTooltips, EditorView, hasHoverTooltips, hoverTooltip, type KeyBinding, keymap, showTooltip, type Tooltip, ViewPlugin, type ViewUpdate } from "@codemirror/view";
import { highlightCode } from "@lezer/highlight";
import type { HoverParams, ServerCapabilities, SignatureHelpContext, SignatureHelpParams } from "vscode-languageserver-protocol";
import type { Hover, SignatureHelp as LspSignatureHelp, MarkedString, MarkupContent } from "vscode-languageserver-types";
...
const pluginHoverLanguageLoads = new WeakMap<Mode, Promise<Language | null>>();
function clientHasCapability(client: LSPClient, name: keyof ServerCapabilities): boolean {
...
function findLanguageDescription(language: string): LanguageDescription | null {
  for (const candidate of getLanguageCandidates(language)) {
    const byName = LanguageDescription.matchLanguageName(languages, candidate, false);
    if (byName) return byName;
    const byExtension = LanguageDescription.matchFilename(languages, `file.${candidate}`);
...
function startPluginLanguageLoad(mode: Mode): Promise<Language | null> | null {
  ...
  const load = Promise.resolve().then(() => loader()).then(extension => {
...
export function resolveLspHoverHighlightLanguage(language: string): Language | null {
  ...
  if (description) {
    ...
    if (!hoverLanguageLoads.has(key)) {
      hoverLanguageLoads.set(key, description.load().then(support => support.language).catch(() => null));
    }
    return null;
  }
...
export async function loadLspHoverHighlightLanguage(language: string): Promise<Language | null> {
  const description = findLanguageDescription(language);
  if (description) {
    if (description.support) return description.support.language;
    const key = description.name.toLowerCase();
    ...
    if (!load) {
      load = description.load().then(support => support.language).catch(() => null);
...
function collectMarkdownLanguages(markdown: string, result: Set<string>): void {
  ...
  for (let match = fencePattern.exec(markdown); match; match = fencePattern.exec(markdown)) {
    const language = getFenceLanguage(match[1] || "");
    if (language) result.add(language);
  }
}
function collectHoverLanguages(contents: Hover["contents"], result = new Set<string>()): Set<string> {
...
async function loadHoverContentLanguages(contents: Hover["contents"]): Promise<void> {
  ...
  await Promise.all(Array.from(languageTags, language => loadLspHoverHighlightLanguage(language)));
}
function fromPosition(doc: EditorView["state"]["doc"], position: {
  line: number;
  character: number;
}): number {
...
function renderCode(plugin: LSPPlugin, code: MarkedString): string {
  ...
  let result = "";
  highlightCode(value, lang.parser.parse(value), {
...
function renderTooltipContent(plugin: LSPPlugin, value: string | MarkupContent | MarkedString | MarkedString[]): string {
  if (Array.isArray(value)) {
    return value.map(item => renderCode(plugin, item)).join("<br>");
  }
  if (typeof value === "string" || typeof value === "object" && value != null && "language" in value) {
    return renderCode(plugin, value);
  }
  return plugin.docToHTML(value);
}
function isPointerOrTouchSelection(update: ViewUpdate): boolean {
  return update.selectionSet && update.transactions.some(tr => tr.isUserEvent("pointer") || tr.isUserEvent("select.pointer") || tr.isUserEvent("touch") || tr.isUserEvent("select.touch"));
}
...
function hoverRequest(plugin: LSPPlugin, pos: number) {
  ...
  plugin.client.sync();
  return plugin.client.request<HoverParams, Hover | null>("textDocument/hover", {
...
function lspTooltipSource(view: EditorView, pos: number): Promise<Tooltip | null> {
  const plugins = LSPPlugin.getAll(view, "hover").filter(plugin => clientHasCapability(plugin.client, "hoverProvider"));
  if (!plugins.length) return Promise.resolve(null);
  return Promise.allSettled(plugins.map(plugin => hoverRequest(plugin, pos))).then(async settled => {
    const results: Array<{
      plugin: LSPPlugin;
      result: Hover;
    }> = [];
    ...
    return {
      ...
      create() {
        ...
        for (let index = 0; index < results.length; index++) {
          ...
          section.innerHTML = renderTooltipContent(results[index].plugin, results[index].result.contents);
        }
        return {
          dom
        };
...
const closeHoverOnInteraction = ViewPlugin.fromClass(class {
...
function getSignatureHelp(plugin: LSPPlugin, pos: number, context: SignatureHelpContext) {
  if (!clientHasCapability(plugin.client, "signatureHelpProvider")) {
    return Promise.resolve(null);
  }
  plugin.client.sync();
  return plugin.client.request<SignatureHelpParams, LspSignatureHelp | null>("textDocument/signatureHelp", {
    context,
    position: plugin.toPosition(pos),
    textDocument: {
      uri: plugin.uri
...
function sameActiveParam(a: LspSignatureHelp, b: LspSignatureHelp, active: number): boolean {
  const current = a.signatures[active];
  const next = b.signatures[active];
  if (!current || !next) return false;
  return (current.activeParameter ?? a.activeParameter) === (next.activeParameter ?? b.activeParameter);
}
class SignatureState {
  constructor(readonly data: LspSignatureHelp, readonly active: number, readonly tooltip: Tooltip, readonly client: LSPClient) {}
}
...
function signatureTooltip(data: LspSignatureHelp, active: number, pos: number, client: LSPClient): Tooltip {
  return {
    pos,
    above: true,
  ...
  update(value, tr) {
    for (const effect of tr.effects) {
      if (effect.is(signatureEffect)) {
        if (effect.value) {
          return new SignatureState(effect.value.data, effect.value.active, signatureTooltip(effect.value.data, effect.value.active, effect.value.pos, effect.value.client), effect.value.client);
        }
        return null;
      }
    }
...
function drawSignatureTooltip(view: EditorView, data: LspSignatureHelp, active: number, client: LSPClient) {
  const dom = document.createElement("div");
  ...
  const activeParam = activeParamIndex != null && signature.parameters ? signature.parameters[activeParamIndex] : null;
...
const signaturePlugin = ViewPlugin.fromClass(class {
  ...
  update(update: ViewUpdate) {
    ...
    const plugin = LSPPlugin.getAll(update.view, "signatureHelp").find(candidate => clientHasCapability(candidate.client, "signatureHelpProvider"));
    if (!plugin) return;
  ...
  scheduleRequest(plugin: LSPPlugin, context: SignatureHelpContext, delay: number) {
  ...
  startRequest(plugin: LSPPlugin, context: SignatureHelpContext) {
    ...
    getSignatureHelp(plugin, pos, context).then(result => {
      ...
      if (result && result.signatures.length) {
        ...
        const active = same && context.triggerKind === 3 ? current!.active : result.activeSignature ?? 0;
    ...
    }, context.triggerKind === 1 ? error => plugin.reportError("Signature request failed", error) : undefined);
  }
  close() {
    if (this.delayedRequest) {
      clearTimeout(this.delayedRequest);
...
export const showSignatureHelp: Command = view => {
  ...
  const lspPlugin = LSPPlugin.getAll(view, "signatureHelp").find(candidate => clientHasCapability(candidate.client, "signatureHelpProvider"));
...
const defaultHoverTooltipsExtension: Extension = [hoverTooltip(lspTooltipSource, {
  hideOnChange: true
...
export function signatureHelp(config: {
  ...
  return [signatureState, signaturePlugin, config.keymap === false ? [] : Prec.high(keymap.of(signatureKeymap))];
}
