...
function getRename(plugin: LSPPlugin, pos: number, newName: string) {
  return plugin.client.request<RenameParams, lsp.WorkspaceEdit | null>("textDocument/rename", {
    newName,
...
function getPrepareRename(plugin: LSPPlugin, pos: number) {
  ...
  }, PrepareRenameResponse | lsp.Range | null>("textDocument/prepareRename", {
    position: plugin.toPosition(pos),
    textDocument: {
      uri: plugin.uri
    }
...
async function performRename(view: EditorView): Promise<boolean> {
  ...
  const plugin = LSPPlugin.getAll(view, "rename").find(candidate => !!candidate.client.serverCapabilities?.renameProvider);
  if (!plugin) {
    return false;
  }
  ...
  let canRename = true;
  const supportsPrepare = typeof renameProvider === "object" && renameProvider !== null && "prepareProvider" in renameProvider && renameProvider.prepareProvider === true;
  if (supportsPrepare) {
    try {
      plugin.client.sync();
      ...
      if (prepareResult === null) {
        ...
        } else if ("defaultBehavior" in prepareResult && prepareResult.defaultBehavior) {
          initialValue = word;
  ...
  const newName = await prompt(strings["new name"] || "New name", initialValue, "text", {
    required: true,
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : "Failed to rename symbol";
    const alert = (await import("dialogs/alert")).default;
...
}, pos: lsp.Position): number {
  const line = doc.line(pos.line + 1);
  return line.from + pos.character;
}
async function applyChangesToFile(workspace: AcodeWorkspace, uri: string, lspChanges: LspChange[], mapping: {
...
async function doRename(plugin: LSPPlugin, newName: string, position: number): Promise<void> {
  plugin.client.sync();
  const response = await plugin.client.withMapping(mapping => getRename(plugin, position, newName).then(response => {
  ...
  if (workspaceEdit.changes) {
    for (const uri in workspaceEdit.changes) {
      ...
      const success = await applyChangesToFile(workspace, uri, lspChanges, mapping);
  ...
  if (workspaceEdit.documentChanges) {
    for (const docChange of workspaceEdit.documentChanges) {
      if ("textDocument" in docChange && "edits" in docChange) {
        ...
        const success = await applyChangesToFile(workspace, uri, edits, mapping);
        if (success) filesChanged++;
      }
    }
  }
  console.info(`[LSP:Rename] Renamed to "${newName}" in ${filesChanged} file(s)`);
