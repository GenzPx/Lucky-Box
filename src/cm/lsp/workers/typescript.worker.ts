...
import { CompletionItemKind, DiagnosticSeverity, DiagnosticTag, DocumentHighlightKind, FoldingRangeKind, InlayHintKind, Range, SelectionRange, SymbolKind, type CodeAction, type CodeActionContext, type CompletionItem, type Diagnostic, type DocumentHighlight, type DocumentSymbol, type FoldingRange, type FormattingOptions, type InlayHint, type Location, type LocationLink, type MarkupContent, type Position, type SelectionRange as LspSelectionRange, type SignatureHelp, type TextEdit, type WorkspaceEdit } from "vscode-languageserver-types";
import { getTextDocument, METHOD_NOT_HANDLED, startWorkerServer } from "./protocol";
...
class TypeScriptHost implements ts.LanguageServiceHost {
  constructor(private readonly documents: Map<string, TextDocument>, private readonly options: ts.CompilerOptions, private readonly projectVersion: () => string) {}
  ...
  resolveModuleNames(moduleNames: string[], containingFile: string): (ts.ResolvedModule | undefined)[] {
    return moduleNames.map(moduleName => {
      const openDocument = this.resolveOpenDocument(moduleName, containingFile);
      if (openDocument) return openDocument;
      return ts.resolveModuleName(moduleName, containingFile, this.options, this).resolvedModule;
    });
  }
  private resolveOpenDocument(moduleName: string, containingFile: string): ts.ResolvedModule | undefined {
...
startWorkerServer(async ({
  ...
  const host = new TypeScriptHost(documents, compilerOptions, getProjectVersion);
  ...
  return {
    ...
    validate(document) {
      const diagnostics: ts.Diagnostic[] = [...service.getSyntacticDiagnostics(document.uri), ...service.getSuggestionDiagnostics(document.uri)];
      if (isTypeScriptDocument(document)) {
        diagnostics.push(...service.getSemanticDiagnostics(document.uri));
      }
      return diagnostics.map(diagnostic => toDiagnostic(document, diagnostic));
    },
    request(method, params) {
      if (method === "completionItem/resolve") {
        return resolveCompletion(service, documents, params as CompletionItem);
      }
      const document = getTextDocument(documents, params);
      if (!document) return null;
      const request = params as RequestParams;
      const offset = request.position ? document.offsetAt(request.position) : 0;
      switch (method) {
        case "acode/validate":
          {
            const diagnostics: ts.Diagnostic[] = [...service.getSyntacticDiagnostics(document.uri), ...service.getSuggestionDiagnostics(document.uri)];
        ...
        case "textDocument/signatureHelp":
          return signatureHelp(service, document, offset, request.context);
        case "textDocument/formatting":
          return format(service, document, undefined, request.options);
        case "textDocument/rangeFormatting":
          return format(service, document, request.range, request.options);
        ...
        case "textDocument/prepareRename":
          {
            ...
            return info.canRename ? rangeFromSpan(document, info.triggerSpan) : null;
          }
        case "textDocument/rename":
          return rename(service, documents, document, offset, request.newName);
        case "textDocument/documentHighlight":
          return documentHighlights(service, document, offset);
        case "textDocument/codeAction":
          return codeActions(service, documents, document, request.range!, request.context, request.options);
        case "textDocument/foldingRange":
          return foldingRanges(service, document);
        case "textDocument/selectionRange":
          return selectionRanges(service, document, (params as {
            positions: Position[];
          }).positions);
...
async function loadCompilerOptions(rootUri: string | null | undefined, requestFile: (uri: string) => Promise<string>): Promise<ts.CompilerOptions> {
  ...
  try {
    ...
    const converted = ts.convertCompilerOptionsFromJson(parsed.config.compilerOptions, base);
    return {
      ...defaults,
      ...converted.options,
      noEmit: true
...
function completions(service: ts.LanguageService, document: TextDocument, offset: number) {
  ...
  return {
    ...
    items: result.entries.filter(entry => entry.name).map((entry): CompletionItem => {
      const item: CompletionItem = {
        ...
        commitCharacters: entry.commitCharacters ?? result.defaultCommitCharacters,
        data: {
          acodeLspProvider: "typescript",
          uri: document.uri,
          offset,
...
function resolveCompletion(service: ts.LanguageService, documents: Map<string, TextDocument>, item: CompletionItem): CompletionItem {
  const data = item.data as CompletionData | undefined;
  if (!data || !documents.has(data.uri)) return item;
  const details = service.getCompletionEntryDetails(data.uri, data.offset, data.name, formatSettings({
    tabSize: 4,
    insertSpaces: true
  }), data.source, {
    allowRenameOfImportPath: true,
  ...
  item.documentation = markdownContent([documentation, tags].filter(Boolean).join("\n\n"));
...
function hover(service: ts.LanguageService, document: TextDocument, offset: number) {
  ...
  return {
    ...
    contents: [{
      language: isTypeScriptDocument(document) ? "typescript" : "javascript",
      value: signature
    }, [documentation, tags].filter(Boolean).join("\n\n")]
  };
}
function signatureHelp(service: ts.LanguageService, document: TextDocument, offset: number, context: RequestParams["context"]): SignatureHelp | null {
  ...
  return {
    ...
    signatures: items.items.map(item => ({
      ...
      documentation: markdownContent(ts.displayPartsToString(item.documentation)),
      parameters: item.parameters.map(parameter => ({
        label: ts.displayPartsToString(parameter.displayParts),
        documentation: markdownContent(ts.displayPartsToString(parameter.documentation))
      }))
    }))
  };
}
...
function format(service: ts.LanguageService, document: TextDocument, range: Range | undefined, options?: FormattingOptions): TextEdit[] {
  const settings = formatSettings(options);
  const edits = range ? service.getFormattingEditsForRange(document.uri, document.offsetAt(range.start), document.offsetAt(range.end), settings) : service.getFormattingEditsForDocument(document.uri, settings);
  return edits.map(edit => ({
    range: rangeFromSpan(document, edit.span),
    newText: edit.newText
  }));
}
function documentSymbols(service: ts.LanguageService, document: TextDocument): DocumentSymbol[] {
...
function definitions(service: ts.LanguageService, documents: Map<string, TextDocument>, document: TextDocument, offset: number): LocationLink[] {
  ...
  for (const definition of result.definitions) {
    ...
    links.push({
      ...
      targetRange: definition.contextSpan ? rangeFromSpan(target, definition.contextSpan) : selection,
...
function references(service: ts.LanguageService, documents: Map<string, TextDocument>, document: TextDocument, offset: number): Location[] {
  const result: Location[] = [];
  for (const reference of service.getReferencesAtPosition(document.uri, offset) ?? []) {
...
function rename(service: ts.LanguageService, documents: Map<string, TextDocument>, document: TextDocument, offset: number, newName: string): WorkspaceEdit | null {
  const locations = service.findRenameLocations(document.uri, offset, false, false, {
...
function documentHighlights(service: ts.LanguageService, document: TextDocument, offset: number): DocumentHighlight[] {
  const result: DocumentHighlight[] = [];
  for (const item of service.getDocumentHighlights(document.uri, offset, [document.uri]) ?? []) {
...
function codeActions(service: ts.LanguageService, documents: Map<string, TextDocument>, document: TextDocument, range: Range, context: CodeActionContext, options?: FormattingOptions): CodeAction[] {
  const fixes = service.getCodeFixesAtPosition(document.uri, document.offsetAt(range.start), document.offsetAt(range.end), context.diagnostics.map(diagnostic => Number(diagnostic.code)).filter(Number.isFinite), formatSettings(options), {});
  return fixes.map(fix => {
    ...
    return {
      ...
      kind: "quickfix",
      edit: Object.keys(changes).length ? {
        changes
      } : undefined
    };
  });
}
function foldingRanges(service: ts.LanguageService, document: TextDocument): FoldingRange[] {
  ...
  for (const item of service.getOutliningSpans(document.uri)) {
    ...
    result.push({
      ...
      kind: item.kind === ts.OutliningSpanKind.Comment ? FoldingRangeKind.Comment : item.kind === ts.OutliningSpanKind.Region ? FoldingRangeKind.Region : undefined
    });
  }
  return result;
}
function selectionRanges(service: ts.LanguageService, document: TextDocument, positions: Position[]): LspSelectionRange[] {
  const convert = (selection: ts.SelectionRange): LspSelectionRange => SelectionRange.create(rangeFromSpan(document, selection.textSpan), selection.parent ? convert(selection.parent) : undefined);
  return positions.map(position => convert(service.getSmartSelectionRange(document.uri, document.offsetAt(position))));
}
function inlayHints(service: ts.LanguageService, document: TextDocument, range: Range): InlayHint[] {
  const start = document.offsetAt(range.start);
  const end = document.offsetAt(range.end);
  return service.provideInlayHints(document.uri, {
    start,
    length: end - start
  }, {
    ...
    label: hint.text || hint.displayParts?.map(part => part.text).join("") || "",
    kind: hint.kind === ts.InlayHintKind.Parameter ? InlayHintKind.Parameter : InlayHintKind.Type,
    paddingLeft: hint.whitespaceBefore,
    paddingRight: hint.whitespaceAfter
  }));
}
function toDiagnostic(document: TextDocument, diagnostic: ts.Diagnostic): Diagnostic {
...
function rangeFromSpan(document: TextDocument, span: {
  start?: number;
  length?: number;
}): Range {
  const startOffset = span.start ?? 0;
  return Range.create(document.positionAt(startOffset), document.positionAt(startOffset + (span.length ?? 0)));
}
function formatSettings(options?: FormattingOptions | null): ts.FormatCodeSettings {
...
function signatureTrigger(context: RequestParams["context"]): ts.SignatureHelpTriggerReason {
...
function jsDocTag(tag: ts.JSDocTagInfo): string {
  const text = Array.isArray(tag.text) ? tag.text.map(part => part.text).join("") : tag.text ?? "";
  return `*@${tag.name}*${text ? ` — ${text}` : ""}`;
}
function isTypeScriptDocument(document: TextDocument): boolean {
  return document.languageId.startsWith("typescript") || document.languageId === "tsx";
}
