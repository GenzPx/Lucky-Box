import { ClientCapabilities, getLanguageService, type JSONDocument, type LanguageService } from "vscode-json-languageservice";
...
startWorkerServer(({
  ...
  async function requestSchema(uri: string): Promise<string> {
    ...
    if (url.protocol === "http:" || url.protocol === "https:") {
      ...
      if (!response.ok) {
        throw new Error(`Unable to load JSON schema (${response.status} ${response.statusText})`);
      }
  ...
  function getParsed(document: TextDocument, service: LanguageService): JSONDocument {
  ...
  return {
    ...
    validate(document) {
      ...
      return service.doValidation(document, getParsed(document, service), {
        ...
        trailingCommas: document.languageId === "jsonc" ? "warning" : "error"
      });
    },
    async request(method, params) {
      if (method === "completionItem/resolve") {
        return services.json.doResolve(params as Parameters<LanguageService["doResolve"]>[0]);
      }
      ...
      switch (method) {
        ...
        case "textDocument/colorPresentation":
          return service.getColorPresentations(document, json, request.color, request.range!);
        case "textDocument/foldingRange":
          return service.getFoldingRanges(document);
        case "textDocument/selectionRange":
          return service.getSelectionRanges(document, request.positions, json);
        default:
