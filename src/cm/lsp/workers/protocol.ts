...
export interface WorkerLanguageAdapter {
  ...
  request(method: string, params: unknown): unknown | PromiseLike<unknown> | typeof METHOD_NOT_HANDLED;
  configure?(settings: unknown): void;
  closeDocument?(uri: string): void;
  dispose?(): void;
}
type AdapterFactory = (context: WorkerServerContext) => WorkerLanguageAdapter | Promise<WorkerLanguageAdapter>;
const workerScope = globalThis as unknown as WorkerScope;
const documents = new Map<string, TextDocument>();
const validationTimers = new Map<string, ReturnType<typeof setTimeout>>();
const cancelledRequests = new Set<JsonRpcId>();
const hostRequests = new Map<number, {
...
function publishDiagnostics(uri: string, version: number, diagnostics: Diagnostic[]): void {
...
function scheduleValidation(uri: string): void {
  ...
  validationTimers.set(uri, setTimeout(async () => {
    ...
    try {
      ...
      sendLog("warn", `Validation failed for ${uri}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }, 180));
}
...
function handleNotification(message: JsonRpcMessage): void {
  switch (message.method) {
    ...
    case "workspace/didChangeConfiguration":
      adapter?.configure?.((message.params as {
        settings?: unknown;
      } | null)?.settings);
...
export function getTextDocument(documentsMap: Map<string, TextDocument>, params: unknown): TextDocument | null {
  const uri = (params as {
    textDocument?: {
      uri?: unknown;
    };
  } | null)?.textDocument?.uri;
  return typeof uri === "string" ? documentsMap.get(uri) ?? null : null;
}
export function resolveReference(reference: string, baseUri: string): string | undefined {
  try {
