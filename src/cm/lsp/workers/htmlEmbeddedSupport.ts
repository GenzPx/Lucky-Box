...
export function clearDocumentRegions(uri?: string): void {
  if (uri) cache.delete(uri);else cache.clear();
}
export function getDocumentRegions(languageService: LanguageService, document: TextDocument): HTMLDocumentRegions {
  ...
  if (cached && cached.version === document.version && cached.expires > Date.now()) {
  ...
  return {
    ...
    hasEmbeddedLanguage: (languageId, ignoreAttributeValues) => regions.some(region => region.languageId === languageId && (!ignoreAttributeValues || !region.attributeValue))
  };
}
function getEmbeddedLanguages(regions: EmbeddedRegion[], ignoreAttributeValues?: boolean): string[] {
  ...
  for (const {
    ...
    if (languageId && (!ignoreAttributeValues || !attributeValue) && !result.includes(languageId)) {
...
function getEmbeddedDocument(document: TextDocument, regions: EmbeddedRegion[], languageId: string, ignoreAttributeValues: boolean): string | null {
  ...
  for (const region of regions) {
    if (region.languageId === languageId && (!ignoreAttributeValues || !region.attributeValue)) {
      result = substituteWithWhitespace(result, currentPosition, region.start, documentText, lastSuffix, getPrefix(region));
      result += updateContent(region, documentText.substring(region.start, region.end));
...
function substituteWithWhitespace(result: string, start: number, end: number, oldContent: string, before: string, after: string): string {
  result += before;
