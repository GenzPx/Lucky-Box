...
export class NestedLspClient {
  ...
  constructor(url: string, config: {
    ...
    this.ready = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        const error = new Error(`Timed out starting ${config.serverId} worker`);
        this.fail(error);
        reject(error);
      }, 10000);
      this.worker.onmessage = (event: MessageEvent<unknown>) => {
        ...
        if (data && typeof data === "object") {
          ...
          if (kind === "error") {
            clearTimeout(timeout);
            const message = typeof (data as {
              message?: unknown;
            }).message === "string" ? (data as {
              message: string;
            }).message : "Nested worker failed";
  ...
  async request(method: string, params: unknown, document?: TextDocument): Promise<unknown> {
    await this.ready;
    this.throwIfClosed();
  ...
  private async handleMessage(data: unknown): Promise<void> {
    ...
    if (!hostRequest || hostRequest.kind !== "host-request" || hostRequest.method !== "readFile") {
      return;
    }
    try {
