...
import { formatCommand, quoteArg, runForegroundCommand, runQuickCommand } from "./installRuntime";
import { buildAxsBridgeCommand, checkAxsBridgeStatus, checkServerAliveViaWebSocket } from "./runtimes/axsBridge";
import { getServerBundle } from "./serverCatalog";
import notificationManager from "lib/notificationManager";
import { addLspLog } from "./logs";
import type { InstallCheckResult, InstallStatus, LauncherConfig, LspServerDefinition, LspServerStats, LspServerStatsFormatted, ManagedServerEntry, PortInfo, WaitOptions } from "./types";
...
function getTerminalRequiredMessage(): string {
  return strings?.terminal_required_message_for_lsp ?? "Terminal not installed. Please install Terminal first to use LSP servers.";
}
interface LspError extends Error {
  code?: string;
}
function getExecutor(): Executor {
  const executor = (globalThis as unknown as {
    Executor?: Executor;
  }).Executor;
  if (!executor) {
...
async function getTerminalDataDirs(): Promise<string[]> {
  ...
  const system = (globalThis as unknown as {
    system?: {
      getFilesDir: (success: (filesDir: string) => void, error: (error: string) => void) => void;
    };
  }).system;
  ...
  return new Promise((resolve, reject) => {
    system.getFilesDir((filesDir: string) => {
      cachedFilesDir = filesDir;
      resolve([`${filesDir}/public`, `${filesDir}/alpine/home`]);
    }, (error: string) => reject(new Error(error)));
  });
}
async function getPortFilePaths(serverName: string, session: string): Promise<string[]> {
  const dataDirs = await getTerminalDataDirs();
  const baseName = serverName.split("/").pop() || serverName;
  return dataDirs.map(dataDir => `file://${dataDir}/.axs/lsp_ports/${baseName}_${session}`);
}
...
export async function getLspPort(serverName: string, session: string): Promise<PortInfo | null> {
  try {
...
async function waitForServerReady(serverId: string, timeout = 10000): Promise<boolean> {
...
async function waitForPort(serverId: string, serverName: string, session: string, timeout = 10000): Promise<PortInfo | null> {
  const ready = await waitForServerReady(serverId, timeout);
  if (!ready) {
    console.warn(`[LSP:${serverId}] Server did not signal ready within timeout`);
  }
...
export async function canReuseExistingServer(server: LspServerDefinition, session: string): Promise<number | null> {
  const bridge = server.launcher?.bridge;
  const serverName = resolveServerExecutable(server) || bridge?.command || server.launcher?.command || server.id;
  const portInfo = await getLspPort(serverName, session);
  if (!portInfo) {
    return null;
  }
  ...
  const alive = status === "alive" || status === "unsupported" && (await checkServerAliveViaWebSocket(url, 1000));
  if (alive) {
    addLspLog(server.id, "info", `Reusing existing server on port ${portInfo.port}`);
    console.info(`[LSP:${server.id}] Reusing existing server on port ${portInfo.port}`);
    return portInfo.port;
  }
  console.info(`[LSP:${server.id}] Found stale port file, will start new server`);
  addLspLog(server.id, "warn", "Found stale port file, starting a new server");
  return null;
}
function resolveStartCommand(server: LspServerDefinition, session?: string): string | null {
...
function getInstallCacheKey(server: LspServerDefinition): string | null {
  const checkCommand = server.launcher?.checkCommand || buildDerivedCheckCommand(server);
...
function normalizeInstallSpec(server: LspServerDefinition) {
  ...
  const packages = Array.isArray(install.packages) ? install.packages.map(entry => String(entry || "").trim()).filter(Boolean) : [];
  const kind = install.kind || (install.binaryPath ? "manual" : null) || (install.source === "apk" ? "apk" : null) || (install.source === "npm" ? "npm" : null) || (install.source === "pip" ? "pip" : null) || (install.source === "cargo" ? "cargo" : null) || (install.command ? "shell" : null) || "shell";
  return {
    ...install,
    kind,
    packages,
    command: typeof install.command === "string" && install.command.trim() ? install.command.trim() : undefined,
    updateCommand: typeof install.updateCommand === "string" && install.updateCommand.trim() ? install.updateCommand.trim() : undefined,
    source: install.source || (kind === "shell" ? "custom" : kind === "manual" ? "manual" : kind),
    executable: typeof install.executable === "string" && install.executable.trim() ? install.executable.trim() : undefined,
    binaryPath: typeof install.binaryPath === "string" && install.binaryPath.trim() ? install.binaryPath.trim() : undefined,
    repo: typeof install.repo === "string" && install.repo.trim() ? install.repo.trim() : undefined,
    assetNames: install.assetNames && typeof install.assetNames === "object" ? Object.fromEntries(Object.entries(install.assetNames).map(([key, value]) => [String(key), String(value || "").trim()]).filter(([, value]) => Boolean(value))) : {},
    archiveType: install.archiveType === "binary" ? "binary" : "zip",
    extractFile: typeof install.extractFile === "string" && install.extractFile.trim() ? install.extractFile.trim() : undefined,
    npmCommand: typeof install.npmCommand === "string" && install.npmCommand.trim() ? install.npmCommand.trim() : "npm",
    pipCommand: typeof install.pipCommand === "string" && install.pipCommand.trim() ? install.pipCommand.trim() : "pip",
    pythonCommand: typeof install.pythonCommand === "string" && install.pythonCommand.trim() ? install.pythonCommand.trim() : "python3",
...
function resolveServerExecutable(server: LspServerDefinition): string | null {
  return getProviderExecutable(server) || getInstallerExecutable(server) || server.launcher?.bridge?.command || server.launcher?.command || null;
}
function getInstallLabel(server: LspServerDefinition): string {
  return (normalizeInstallSpec(server)?.label || server.launcher?.install?.label || server.label || server.id).trim();
}
function buildUninstallCommand(server: LspServerDefinition): string | null {
  ...
  switch (spec.kind) {
    case "apk":
      return spec.packages.length ? `apk del ${spec.packages.map(entry => quoteArg(entry)).join(" ")}` : null;
    case "npm":
      {
        if (!spec.packages.length) return null;
        const npmCommand = spec.npmCommand || "npm";
        const uninstallFlags = spec.global !== false ? "uninstall -g" : "uninstall";
        return `${npmCommand} ${uninstallFlags} ${spec.packages.map(entry => quoteArg(entry)).join(" ")}`;
      }
    case "pip":
      return spec.packages.length ? `${spec.pipCommand || "pip"} uninstall -y ${spec.packages.map(entry => quoteArg(entry)).join(" ")}` : null;
    case "cargo":
      return spec.packages.length ? spec.packages.map(entry => `cargo uninstall ${quoteArg(entry)}`).join(" && ") : null;
    case "github-release":
    case "manual":
...
function buildInstallCommand(server: LspServerDefinition, mode: "install" | "update" = "install"): string | null {
  const spec = normalizeInstallSpec(server);
  if (!spec) return null;
  ...
  switch (spec.kind) {
    case "apk":
      return spec.packages.length ? `apk add --no-cache ${spec.packages.map(entry => quoteArg(entry)).join(" ")}` : null;
    case "npm":
      {
        if (!spec.packages.length) return null;
        const npmCommand = spec.npmCommand || "npm";
    ...
    case "pip":
      {
        ...
        const breakPackages = spec.breakSystemPackages !== false ? "PIP_BREAK_SYSTEM_PACKAGES=1 " : "";
        return `apk add --no-cache python3 py3-pip && ${breakPackages}${pipCommand} install ${spec.packages.map(entry => quoteArg(entry)).join(" ")}`;
      }
    case "cargo":
      return spec.packages.length ? `apk add --no-cache rust cargo && cargo install ${spec.packages.map(entry => quoteArg(entry)).join(" ")}` : null;
    case "github-release":
      {
...
function getUpdateCommand(server: LspServerDefinition): string | null {
  const launcher = server.launcher;
  if (!launcher) return null;
  if (typeof launcher.updateCommand === "string" && launcher.updateCommand.trim()) {
    return launcher.updateCommand.trim();
  }
  return buildInstallCommand(server, "update");
}
async function readServerVersion(server: LspServerDefinition): Promise<string | null> {
...
export function getInstallCommand(server: LspServerDefinition, mode: "install" | "update" = "install"): string | null {
  if (mode === "update") {
...
export function getUninstallCommand(server: LspServerDefinition): string | null {
  return buildUninstallCommand(server);
}
export async function checkServerInstallation(server: LspServerDefinition): Promise<InstallCheckResult> {
  const bundle = getServerBundle(server.id);
  ...
  const checkCommand = launcher?.checkCommand || buildDerivedCheckCommand(server);
  if (!checkCommand) {
    return {
      status: "unknown",
...
export async function installServer(server: LspServerDefinition, mode: InstallActionMode = "install", options: {
  ...
  const command = mode === "install" ? getInstallCommand(server, "install") : getUpdateCommand(server);
  if (!command) {
    throw new Error(`${displayLabel} has no ${actionLabel.toLowerCase()} command.`);
  }
  if (promptConfirm) {
    ...
    notificationManager.pushNotification({
      icon: "zap",
      title: displayLabel,
      message: strings["lsp-install-notification"].replace("{server}", displayLabel),
      type: "warning",
      action: async (notification: { id: string }) => {
  ...
  try {
    ...
    toast(result.status === "present" ? `${displayLabel} ${isUpdate ? "updated" : "installed"}` : `${displayLabel} ${actionLabel.toLowerCase()} finished`);
    return true;
  } catch (error) {
...
export async function uninstallServer(server: LspServerDefinition, options: {
  promptConfirm?: boolean;
} = {}): Promise<boolean> {
  const bundle = getServerBundle(server.id);
  ...
  if (promptConfirm) {
    notificationManager.pushNotification({
      icon: "zap",
      title: displayLabel,
      message: strings["lsp-uninstall-notification"].replace("{server}", displayLabel),
      type: "warning",
      action: (notification: { id: string }) => {
...
async function performInstallCheck(server: LspServerDefinition, launcher: LauncherConfig | undefined, cacheKey: string): Promise<boolean> {
  try {
    const checkCommand = launcher?.checkCommand || buildDerivedCheckCommand(server);
    if (checkCommand) {
      await runQuickCommand(checkCommand);
    ...
    if (!getInstallCommand(server, "install")) {
      checkedCommands.set(cacheKey, STATUS_FAILED);
      console.warn(`LSP server ${server.id} is missing check command result and has no installer.`, error);
      throw error;
    }
...
async function startInteractiveServer(command: string, serverId: string, logOutput: LauncherConfig["logOutput"] = "all"): Promise<string> {
  const executor = getExecutor();
  const callback: ExecutorCallback = (type, data) => {
    ...
    if (logOutput === "warnings-and-errors") {
      const level = /\b(error|failed|failure|fatal|panic)\b/i.test(data) ? "error" : /\b(warn(?:ing)?|unknown method|disabled|not supported|lacks)\b/i.test(data) ? "warn" : null;
      if (!level) return;
...
async function waitForWebSocket(url: string, options: WaitOptions = {}): Promise<void> {
  const {
    delay = 100,
    probeTimeout = 5000
  } = options;
  ...
  for (const [serverId, entry] of entries) {
    if (entry.command.includes(`--port ${port}`) || entry.command.includes(`:${port}`)) {
  ...
  console.debug(`[LSP] waitForWebSocket timed out for ${url}, proceeding anyway`);
}
export interface EnsureServerResult {
  uuid: string | null;
  discoveredPort?: number;
}
export async function ensureServerRunning(server: LspServerDefinition, session?: string): Promise<EnsureServerResult> {
  const launcher = server.launcher;
  if (!launcher) return {
    uuid: null
  };
  ...
  const serverName = resolveServerExecutable(server) || bridge?.command || launcher.command || server.id;
  try {
    const existingPort = await canReuseExistingServer(server, effectiveSession);
    if (existingPort !== null) {
      return {
        uuid: null,
        discoveredPort: existingPort
      };
    }
  } catch {}
  const terminal = (globalThis as unknown as {
    Terminal?: {
  ...
  if (!isTerminalInstalled) {
    ...
    if (!localStorage.getItem(DONT_ASK_TERMINAL_REQUIRED_FOR_LSP)) {
      if (!alreadyInformed) {
        ...
        if (typeof response === "object" && response.confirmed && response.checked) {
  ...
  if (!installed) {
    const unavailable: LspError = new Error(`Language server ${server.id} is not available.`);
    unavailable.code = "LSP_SERVER_UNAVAILABLE";
    throw unavailable;
  }
  const key = server.id;
  ...
  try {
    const uuid = await startInteractiveServer(command, key, launcher.logOutput);
    let discoveredPort: number | undefined;
    if (bridge && !bridge.port) {
      const portInfo = await waitForPort(key, serverName, effectiveSession, 10000);
      if (portInfo) {
        discoveredPort = portInfo.port;
        console.info(`[LSP:${server.id}] Auto-discovered port ${discoveredPort}`);
      ...
      if (!discoveredPort) {
        throw new Error(`Could not discover websocket bridge port for ${server.id}`);
      }
    } else if (server.transport?.url && (server.transport.kind === "websocket" || server.transport.kind === "stdio")) {
    ...
    const entry = managedServers.get(key);
    if (entry) {
      getExecutor().stop(entry.uuid).catch((err: Error) => {
        console.warn(`Failed to stop language server shell ${server.id}`, err);
      });
      managedServers.delete(key);
    }
    const unavailable: LspError = new Error(`Language server ${server.id} failed to start (${errorMessage})`);
    unavailable.code = "LSP_SERVER_UNAVAILABLE";
    throw unavailable;
  }
}
...
export function resetManagedServers(): void {
  ...
  getExecutor().stopService().catch(() => {});
}
export function getManagedServerInfo(serverId: string): ManagedServerEntry | null {
...
export async function getServerStats(serverId: string, timeout = 2000): Promise<LspServerStatsFormatted | null> {
  const entry = managedServers.get(serverId);
  if (!entry?.port) {
    return null;
  }
