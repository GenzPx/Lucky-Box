...
interface RegistryAdapter {
  registerServer: (definition: LspServerManifest, options?: {
    replace?: boolean;
  }) => unknown;
...
export function registerServerBundle(bundle: LspServerBundle, options: {
  replace?: boolean;
} = {}): LspServerBundle {
  const {
    replace = false
  ...
  for (const definition of definitions) {
    ...
    if (owner && owner !== key && !replace) {
      throw new Error(`LSP server ${serverId} is already provided by ${owner}; ${key} must replace explicitly`);
    }
