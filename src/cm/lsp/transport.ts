...
import type { LspServerDefinition, TransportContext, TransportHandle, WebSocketTransportOptions } from "./types";
...
function createWebSocketTransport(server: LspServerDefinition, context: TransportContext): TransportHandle {
  const transport = server.transport;
  if (!transport) {
    throw new Error(`LSP server ${server.id} is missing transport configuration`);
  }
  ...
  if (context.dynamicPort && context.dynamicPort > 0) {
    ...
    console.info(`[LSP:${server.id}] Using auto-discovered port ${context.dynamicPort}`);
    addLspLog(server.id, "info", `Using auto-discovered port ${context.dynamicPort}`);
  }
  if (!url) {
    throw new Error(`WebSocket transport for ${server.id} has no URL (and no dynamic port available)`);
  }
  ...
  function createSocket(): WebSocket {
    try {
      ...
      throw new Error(`Failed to construct WebSocket for ${server.id} (${wsUrl}): ${message}`);
    }
  }
      ...
      if (msg && typeof msg.id !== "undefined") {
        ...
        switch (msg.method) {
          ...
          case "workspace/configuration":
            result = Array.isArray(msg.params?.items) ? msg.params.items.map((item: {
              section?: unknown;
          ...
          case "workspace/workspaceFolders":
            {
              ...
              result = rootUri ? [{
                uri: rootUri,
                name: rootUri.replace(/\/$/, "").split("/").pop() || rootUri
              }] : null;
              break;
            }
          default:
        ...
        if (msg.method === "workspace/diagnostic/refresh") {
          notifyListeners(JSON.stringify({
            jsonrpc: "2.0",
            method: msg.method,
            params: msg.params ?? {}
          }));
  ...
  function handleClose(event: CloseEvent): void {
    ...
    console.warn(`[LSP:${server.id}] WebSocket closed unexpectedly (code: ${event.code})`);
    addLspLog(server.id, "warn", `WebSocket closed unexpectedly (code: ${event.code})`);
  ...
  function handleError(event: Event): void {
    ...
    const reason = errorEvent?.message || errorEvent?.type || "connection error";
  ...
  function scheduleReconnect(): void {
    ...
    reconnectAttempts++;
    console.info(`[LSP:${server.id}] Reconnecting in ${delay}ms (attempt ${reconnectAttempts}/${maxReconnectAttempts})`);
...
function createStdioTransport(server: LspServerDefinition, context: TransportContext): TransportHandle {
  if (!server.transport) {
    throw new Error(`LSP server ${server.id} is missing transport configuration`);
  }
  if (!server.transport.url && !(context.dynamicPort && context.dynamicPort > 0)) {
    throw new Error(`STDIO transport for ${server.id} is missing a websocket bridge url`);
  }
  if (!server.transport.options?.binary) {
    console.info(`LSP server ${server.id} is using stdio bridge without binary mode. Falling back to text frames.`);
  }
  return createWebSocketTransport(server, context);
}
export function createTransport(server: LspServerDefinition, context: TransportContext = {}): TransportHandle {
  if (!server) {
    throw new Error("createTransport requires a server configuration");
  }
  if (!server.transport) {
    throw new Error(`LSP server ${server.id || "unknown"} is missing transport configuration`);
  }
  const kind = server.transport.kind;
  if (!kind) {
    throw new Error(`LSP server ${server.id} transport is missing 'kind' property`);
  }
  switch (kind) {
    case "websocket":
      return createWebSocketTransport(server, context);
    ...
    case "external":
      ...
      throw new Error(`LSP server ${server.id} declares an external transport without a create() factory`);
    default:
