...
export function getArchitectureMatchers(assets: Record<string, string> | undefined | null): Array<{
  canonicalArch: string;
  aliases: string[];
  asset: string;
}> {
  ...
  for (const [rawArch, rawAsset] of Object.entries(assets)) {
    ...
    const aliases = (ARCH_ALIASES[canonicalArch as NormalizedArch] || [canonicalArch]).map(value => String(value));
    resolved.set(canonicalArch, {
      aliases,
      asset
    });
  }
...
export function buildShellArchCase(assets: Record<string, string> | undefined | null, quote: (value: unknown) => string): string {
  return getArchitectureMatchers(assets).map(({
    aliases,
    asset
  }) => `\t${aliases.join("|")}) ASSET=${quote(asset)} ;;`).join("\n");
}
