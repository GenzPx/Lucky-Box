...
function getWorkspaceRuntimeOverride(settings: RuntimeSettings, context: LspRuntimeContext): string | null {
  const workspaces = settings.workspaces;
  if (!workspaces || typeof workspaces !== "object") return null;
  const candidates = [context.rootUri, context.file?.uri, context.uri, context.documentUri].map(uri => String(uri || "")).filter(Boolean);
  let bestMatch = "";
  let bestRuntime: string | null = null;
  for (const [prefix, runtimeId] of Object.entries(workspaces)) {
    const normalizedPrefix = String(prefix || "");
    if (!normalizedPrefix) continue;
    if (normalizedPrefix.length > bestMatch.length && candidates.some(uri => uri.startsWith(normalizedPrefix))) {
...
export function getConfiguredRuntimeId(server: LspServerDefinition, context: LspRuntimeContext = {}): string | null {
...
export async function setServerRuntime(serverId: string, runtimeId: string): Promise<void> {
...
export function getServerRuntimeSetting(serverId: string): string {
  const normalizedServerId = String(serverId || "").trim().toLowerCase();
  const servers = (appSettings.value?.lsp?.runtime?.servers || {}) as Record<string, unknown>;
  return toRuntimeId(servers[normalizedServerId]) || AUTO_RUNTIME_ID;
}
