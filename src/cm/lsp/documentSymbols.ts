...
import type { DocumentSymbol, Position, Range, SymbolInformation, SymbolKind } from "vscode-languageserver-types";
...
function processDocumentSymbol(symbol: DocumentSymbol, depth = 0, containerName?: string): ProcessedSymbol {
  ...
  if (symbol.children && symbol.children.length > 0) {
    processed.children = symbol.children.map(child => processDocumentSymbol(child, depth + 1, symbol.name));
  }
  return processed;
}
function processSymbolInformation(symbol: SymbolInformation, depth = 0): ProcessedSymbol {
  return {
    name: symbol.name,
    kind: symbol.kind,
    kindName: getSymbolKindName(symbol.kind),
...
function flattenSymbols(symbols: ProcessedSymbol[], result: FlatSymbol[] = []): FlatSymbol[] {
...
export async function fetchDocumentSymbols(view: EditorView): Promise<ProcessedSymbol[] | null> {
  const plugin = LSPPlugin.getAll(view, "documentSymbol").find(candidate => !!candidate.client.serverCapabilities?.documentSymbolProvider) as LSPPluginAPI | undefined;
  if (!plugin) {
    return null;
  }
  ...
  try {
    const response = await client.request<DocumentSymbolParams, (DocumentSymbol | SymbolInformation)[] | null>("textDocument/documentSymbol", params);
    if (!response || response.length === 0) {
      return [];
    }
    if (isDocumentSymbol(response[0])) {
      return (response as DocumentSymbol[]).map(sym => processDocumentSymbol(sym));
    }
    return (response as SymbolInformation[]).map(sym => processSymbolInformation(sym));
  } catch (error) {
    console.warn("Failed to fetch document symbols:", error);
    return null;
  }
}
export async function getDocumentSymbolsFlat(view: EditorView): Promise<FlatSymbol[]> {
...
export async function navigateToSymbol(view: EditorView, symbol: FlatSymbol | ProcessedSymbol): Promise<boolean> {
  try {
...
export function supportsDocumentSymbols(view: EditorView): boolean {
  return LSPPlugin.getAll(view, "documentSymbol").some(plugin => plugin.client.connected && !!plugin.client.serverCapabilities?.documentSymbolProvider);
}
export interface DocumentSymbolsResult {
  symbols: ProcessedSymbol[];
  flat: FlatSymbol[];
}
export async function getDocumentSymbols(view: EditorView): Promise<DocumentSymbolsResult | null> {
  const symbols = await fetchDocumentSymbols(view);
  if (symbols === null) {
    return null;
  }
