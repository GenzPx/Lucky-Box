...
export default function () {
	...
	const list = new Ref();
	let cmPreview = null;
	const previewDoc = `// LuckyBox is ready!\nconst message = "Welcome to LuckyBox";\nconsole.log(message);`;
		...
		const container = (
			...
			>
				...
				<div className="terminal-line terminal-prompt">
					...
					<span style={`color:${theme.foreground};`}>:~$ </span>
					<span>echo "Hello, LuckyBox!"</span>
				</div>
				<div className="terminal-line terminal-output">Hello, LuckyBox!</div>
				<div className="terminal-line terminal-prompt">
					<span style={`color:${theme.green};`}>user</span>
					<span style={`color:${theme.foreground};`}>@</span>
					<span style={`color:${theme.blue};`}>acode</span>
	...
	function setEditorTheme({ caption, theme }) {
		...
		if (!ok) {
			alert(
				"Invalid theme",
				"This editor theme is not compatible with LuckyBox's CodeMirror runtime.",
			);
			return;
		}
