...
export default function PluginsInclude(updates) {
	...
	const LIMIT = 50;
	const isFreePlugin = (plugin) => Number(plugin?.price || 0) <= 0;
	...
	async function searchRemotely(query) {
		...
		try {
			...
			return plugins.filter(isFreePlugin).map((plugin) => <Item {...plugin} />);
		} catch (error) {
	...
	async function retrieveFilteredPlugins(filterState) {
		...
		if (filterState.type === "orderBy") {
			...
			try {
				...
				const items = rawItems.filter(isFreePlugin);
				const hasMoreResults = rawItems.length === LIMIT;
				return {
					items,
					hasMore: hasMoreResults,
				};
		...
		while (items.length < LIMIT) {
			...
			try {
				...
				const matched = data.filter(
					(plugin) =>
						isFreePlugin(plugin) && matchesFilter(plugin, filterState),
				);
		...
		return {
			items,
			hasMore: Boolean(hasMoreResults),
		};
	}
	...
	async function getAllPlugins() {
		...
		try {
			...
			const rawPlugins = await response.json();
			const newPlugins = Array.isArray(rawPlugins)
				? rawPlugins.filter(isFreePlugin)
				: [];
			if (!Array.isArray(rawPlugins) || rawPlugins.length < LIMIT) {
				hasMore = false;
			}
			const installed = await fsOperation(PLUGIN_DIR).lsDir();
			const disabledMap = settings.value.pluginsDisabled || {};
	...
	async function getInstalledPlugins(updates) {
		...
		await Promise.all(
			installed.map(async (item) => {
				...
				plugin.installed = true;
				plugin.enabled = disabledMap[id] !== true;
				plugin.onToggleEnabled = onToggleEnabled;
