...
export async function runCodeMirrorTests(writeOutput) {
	...
	function dispatchQuickToolsTextInput(text) {
		...
		quickTools.$input.dispatchEvent(
			new Event("input", {
				bubbles: true,
			}),
		);
	...
	runner.test(
		...
		async (test) => {
			await withEditor(test, async (view) => {
				...
				const handled = runQuickToolNavigation(view, 39, {
					shiftKey: false,
				});
				const main = view.state.selection.main;
	...
	runner.test("Quick tools Shift+Home selects to line start", async (test) => {
		await withEditor(test, async (view) => {
			...
			const handled = runQuickToolKey(view, 36, {
				shiftKey: true,
			});
			const main = view.state.selection.main;
	...
	runner.test(
		...
		async (test) => {
			await withEditor(test, async (view) => {
				...
				const upHandled = runQuickToolNavigation(view, 38, {
					shiftKey: true,
				});
	...
	runner.test("Quick tools modifier combos resolve commands", (test) => {
		...
		test.assertEqual(
			findQuickToolCommand(commands, "a", {
				ctrlKey: true,
			})?.name,
			"selectall",
		);
		test.assertEqual(
			findQuickToolCommand(commands, "s", {
				ctrlKey: true,
			})?.name,
			"saveFile",
		);
		test.assertEqual(
			findQuickToolCommand(commands, "y", {
				ctrlKey: true,
			})?.name,
			"redo",
		);
	...
	runner.test("Scroll API", async (test) => {
		await withEditor(test, async (view) => {
			...
			view.dispatch({
				effects: EditorView.scrollIntoView(line50.from, {
					y: "center",
				}),
			});
			await new Promise((resolve) => setTimeout(resolve, 100));
